...
def test_memory_system():
    ...
    test_memories = [
        ("Machine learning is a subset of AI focused on algorithms that learn from data.", ["ai", "ml", "important"]),
        ("Python is a versatile programming language.", ["programming", "python"]),
        ("Database indexing improves query performance significantly.", ["database", "performance", "important"]),
        ("Microservices architecture promotes scalability and maintainability.", ["architecture", "microservices"]),
        ("Error handling is crucial for robust software systems.", ["error", "programming", "critical"])
    ]
    ...
    for query in queries:
        ...
        print(f"    Query: '{query}'")
        print(f"      Found {len(retrieval.entries)} entries from {len(retrieval.source_layers)} layers")
        print(f"      Total tokens: {retrieval.total_tokens}, Time: {retrieval.retrieval_time:.3f}s")
...
def test_tool_system():
    ...
    for spec in registry.list_tool_specs():
        ...
        print(f"      Rate limit: {spec.rate_limit_per_min}/min, Auth: {spec.auth_required}")
...
async def test_tool_execution(registry):
    ...
    test_cases = [
        ...
        },
        {
            "name": "File Write",
            "tool": "file_write",
            "params": {"filename": "test_output.txt", "content": "P2 test output from AgentNet"}
        }
    ]
...
async def test_integrated_agent():
    ...
    memory_config = {
        "memory": {
            ...
            "episodic": {"enabled": True, "storage_path": "demo_output/agent_episodic.json"},
            "semantic": {"enabled": True, "storage_path": "demo_output/agent_semantic.json"}
        }
    }
