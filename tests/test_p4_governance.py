    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(5):
            cost_recorder.record_inference_cost(
                provider="example",
                model="test-model",
                result={"content": f"Test {i}", "tokens_input": 100, "tokens_output": 50},
        ...
        print(f"  ✅ Cost aggregation: {summary['record_count']} records, ${summary['total_cost']:.6f}")
        ...
        tenant_tracker.set_tenant_alerts("tenant-001", {"warning": 0.75, "critical": 0.90})
        
        budget_status = tenant_tracker.check_tenant_budget("tenant-001")
        assert budget_status["status"] in ["ok", "alert_warning"]
        print(f"  ✅ Tenant budget tracking: {budget_status['status']} - ${budget_status['current_spend']:.6f}/${budget_status['budget']:.2f}")
...
def test_rbac_system():
    ...
    # Test tenant access
    assert rbac_manager.user_can_access_tenant(admin_user, "any-tenant")  # Admin can access any
...
def test_integrated_governance():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create monitors
        semantic_monitor = MonitorFactory.build(MonitorSpec(
            name="semantic_guard",
            type="semantic_similarity",
        ...
        classifier_monitor = MonitorFactory.build(MonitorSpec(
            name="pii_guard",
            type="llm_classifier",
        ...
        if result["cost_record"]:
            ...
            print(f"  ✅ Reasoning completed with cost tracking: ${result['cost_record']['total_cost']:.6f}")
        else:
        ...
        print(f"  ✅ Multiple interactions completed, total estimated cost: ${total_cost:.6f}")
        
        # Final cost summary
        final_summary = aggregator.get_cost_summary(tenant_id="tenant-governed")
        print(f"  ✅ Final cost tracking: {final_summary['record_count']} records, ${final_summary['total_cost']:.6f} total")
