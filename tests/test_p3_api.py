...
def test_dag_execution_endpoint():
    ...
    print(
        f"  ✅ DAG executed: {len(data['completed_tasks'])} tasks completed in {data['total_time'] or 0:.2f}s"
    )
...
def test_invalid_requests():
    ...
    print(
        f"  ✅ Invalid graph rejected: {data.get('validation_errors', ['cycle detected'])}"
    )
