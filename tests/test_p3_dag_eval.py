...
def test_dag_planner():
    ...
    assert execution_order == expected_order, f"Unexpected execution order: {execution_order}"
    ...
    assert analysis['node_count'] == 4, f"Expected 4 nodes, got {analysis['node_count']}"
    assert analysis['max_depth'] == 3, f"Expected depth 3, got {analysis['max_depth']}"
    print(f"  ✅ Graph analysis: {analysis['node_count']} nodes, depth {analysis['max_depth']}")
    ...
    assert any("cycle" in error.lower() for error in cycle_graph.validation_errors), "Should detect cycle"
...
async def test_task_scheduler():
    ...
    async def example_task_executor(task_id: str, prompt: str, agent: str, context: dict) -> dict:
    ...
    assert result.status == "completed", f"Execution should complete, got {result.status}"
    assert len(result.get_completed_tasks()) == 4, f"Expected 4 completed tasks, got {len(result.get_completed_tasks())}"
    assert len(result.get_failed_tasks()) == 0, f"Expected 0 failed tasks, got {len(result.get_failed_tasks())}"
    print(f"  ✅ Basic execution: {len(result.get_completed_tasks())} tasks completed in {result.total_time or 0:.2f}s")
    ...
    for task_id, task_result in result.task_results.items():
        ...
        assert task_result.agent_used is not None, f"Task {task_id} should record agent used"
...
def test_metrics_calculator():
    ...
    assert results.success_rate == 1.0, f"Expected perfect score, got {results.success_rate}"
    assert len(results.criteria_results) == 2, f"Expected 2 criteria results, got {len(results.criteria_results)}"
    ...
    print(f"  ✅ Failed criteria detection: success rate {bad_results.success_rate:.2f}")
    
    # Test 3: Semantic scoring with reference text
    calculator.register_reference_text("ref1", "Network resilience requires redundancy and failover")
    ...
    semantic_results = calculator.evaluate_content(content, semantic_criteria, "semantic_test")
...
async def test_evaluation_runner():
    ...
    async def example_workflow_executor(task_graph, config):
        ...
        return {
            "status": "completed",
            "task_results": {
                "root": {"status": "completed", "result": {"content": "System architecture requirements defined", "confidence": 0.9}},
                "analysis": {"status": "completed", "result": {"content": "Key components analyzed and identified", "confidence": 0.85}}
            }
        }
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert result.status == "completed", f"Scenario should complete, got {result.status}"
        ...
        print(f"  ✅ Single scenario: {result.scenario_name} completed with score {result.metrics.success_rate:.2f}")
        ...
        workflow_scenario = EvaluationScenario(
            ...
            task_graph={
                "nodes": [
                    {"id": "root", "prompt": "Define requirements", "agent": "Planner", "deps": []},
                    {"id": "analysis", "prompt": "Analyze components", "agent": "Analyst", "deps": ["root"]}
                ]
            },
            success_criteria=[
                SuccessCriteria(
        ...
        assert workflow_result.status == "completed", f"Workflow should complete, got {workflow_result.status}"
...
async def test_yaml_evaluation_suite():
    ...
    assert len(suite.scenarios) >= 3, f"Expected at least 3 scenarios, got {len(suite.scenarios)}"
    ...
    if workflow_scenarios:
        ...
        assert workflow.task_graph is not None, "Workflow scenario should have task_graph"
...
async def test_integration_with_agentnet():
    ...
    simple_graph = planner.create_graph_from_dict({
        "nodes": [
            ...
            {"id": "analyze", "prompt": "Analyze the architecture plan for risks", "agent": "Analyst", "deps": ["plan"]}
        ]
    })
    ...
    assert result.status == "completed", f"Integration execution should complete, got {result.status}"
    assert len(result.get_completed_tasks()) == 2, f"Expected 2 completed tasks, got {len(result.get_completed_tasks())}"
    
    # Verify AgentNet-specific content
    for task_id, task_result in result.task_results.items():
        assert "meta_insights" in task_result.result, f"Task {task_id} should have meta_insights from AgentNet"
        assert task_result.result["confidence"] > 0, f"Task {task_id} should have confidence > 0"
    
    print(f"  ✅ AgentNet integration: {len(result.get_completed_tasks())} tasks completed")
