...
def test_metrics_collection():
    ...
    if not metrics.enable_prometheus:
        ...
        expected_metrics = [
            "inference_latency_seconds", "tokens_consumed_total", "violations_total",
            "cost_usd_total", "session_rounds_total", "tool_invocations_total",
            "dag_node_duration_seconds", "active_sessions"
        ]
...
def test_tracing():
    ...
    with tracer.trace_agent_inference("test-agent", "gpt-3.5-turbo", "openai", session_id) as span:
        assert span is not None
    ...
    with tracer.trace_dag_node("node-1", "planning", "planner-agent", session_id) as span:
        assert span is not None
        time.sleep(0.01)
...
def test_structured_logging():
    ...
    from agentnet.observability.logging import (
        setup_structured_logging, get_correlation_logger,
        set_correlation_id, get_correlation_id, clear_correlation_context
    )
...
def test_dashboard():
    ...
    from agentnet.observability.dashboard import (
        DashboardDataCollector, TimeRange, CostSummary, 
        PerformanceMetrics, ViolationSummary, SessionMetrics
    )
    ...
    # Add cost events
    dashboard.add_cost_event("openai", "gpt-3.5-turbo", 0.003, 150, "test-tenant", base_time)
    dashboard.add_cost_event("anthropic", "claude-3", 0.008, 200, "test-tenant", base_time + timedelta(minutes=10))
    
    # Add performance events
    dashboard.add_performance_event("agent-1", "gpt-3.5-turbo", "openai", 120.5, True, base_time)
    dashboard.add_performance_event("agent-2", "claude-3", "anthropic", 150.0, True, base_time + timedelta(minutes=5))
    dashboard.add_performance_event("agent-1", "gpt-3.5-turbo", "openai", 200.0, False, base_time + timedelta(minutes=15))
    
    # Add violation events
    dashboard.add_violation_event("token_limit", "high", "agent-1", "Token limit exceeded", base_time)
    dashboard.add_violation_event("content_filter", "medium", "agent-2", "Content filtered", base_time + timedelta(minutes=20))
    
    # Add session events
    dashboard.add_session_event("session-1", "brainstorm", "start", 0, False, base_time)
    dashboard.add_session_event("session-1", "brainstorm", "round", 1, False, base_time + timedelta(minutes=5))
    dashboard.add_session_event("session-1", "brainstorm", "round", 2, True, base_time + timedelta(minutes=10))
...
def test_integration():
    ...
    from agentnet.observability.logging import get_correlation_logger, set_correlation_id
    ...
    with tracer.trace_session_round(session_id, 1, "test"):
        logger.set_correlation_context(session_id=session_id, operation="integration_test")
