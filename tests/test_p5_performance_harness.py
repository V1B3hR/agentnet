...
async def test_performance_harness():
    ...
    def agent_factory():
        return AgentNet(
            "TestAgent", {"logic": 0.8, "creativity": 0.6}, engine=ExampleEngine()
        )
...
def test_latency_tracker():
    ...
    time.sleep(0.01)
    inference_latency = tracker.end_component_measurement(
        turn_id, LatencyComponent.INFERENCE
    )

    tracker.start_component_measurement(turn_id, LatencyComponent.POLICY_CHECK)
    time.sleep(0.005)
    policy_latency = tracker.end_component_measurement(
        turn_id, LatencyComponent.POLICY_CHECK
    )
    ...
    print(
        f"  🔧 Components: Inference {inference_latency:.2f}ms, Policy {policy_latency:.2f}ms"
    )
...
async def test_integrated_performance_measurement():
    ...
    async def tracked_operation(agent, prompt):
        ...
        inference_latency = latency_tracker.end_component_measurement(
            turn_id, LatencyComponent.INFERENCE
        )
        ...
        latency_tracker.end_turn_measurement(
            turn_id, len(str(result)), input_tokens + output_tokens
        )

        return result

    # Run integrated benchmark
    benchmark_result = await harness.run_benchmark(
        config, agent_factory, tracked_operation
    )
