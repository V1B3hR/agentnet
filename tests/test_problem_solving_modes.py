...
class TestCoreEnums:
    ...
    def test_problem_technique_enum_values(self):
        ...
        expected_techniques = {
            "simple", "complex", "troubleshooting", 
            "gap_from_standard", "target_state", "open_ended"
        }
...
class TestAutoConfigIntegration:
    ...
    def test_autoconfig_data_injection(self):
        ...
        if params.recommended_style:
            assert autoconfig_data["recommended_style"] == params.recommended_style.value
        if params.recommended_technique:
            assert autoconfig_data["recommended_technique"] == params.recommended_technique.value
...
class TestStrategyClasses:
    ...
    def test_strategy_execution(self):
        ...
        result = strategy.execute(
            agent=self.agent,
            task="Generate ideas for testing"
        )
    ...
    def test_all_strategy_types(self):
        ...
        for StrategyClass, expected_mode, expected_focus in strategies:
            ...
            result = strategy.execute(
                agent=self.agent,
                task=f"Test {expected_mode} mode"
            )
