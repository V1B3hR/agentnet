...
from agentnet.critique.evaluator import (
    CritiqueEvaluator,
    RevisionEvaluator,
    CritiqueType,
    RevisionTrigger,
...
class MockAgent:
    ...
    def __init__(
        self, name: str, responses: list = None, role_behavior: str = "neutral"
    ):
        self.name = name
...
async def test_critique_evaluator_basic():
    ...
    evaluator = CritiqueEvaluator(
        name="test_critic", quality_threshold=0.6, enable_automated_scoring=True
    )
    ...
    critique = await evaluator.evaluate_content(
        content=bad_content, context={"confidence": 0.2}
    )
...
async def test_critique_evaluator_with_agent():
    ...
    evaluator = CritiqueEvaluator(
        name="agent_critic", critique_agent=critic_agent, enable_automated_scoring=True
    )
...
async def test_revision_evaluator():
    ...
    revision_evaluator = RevisionEvaluator(
        name="test_revision", improvement_threshold=0.1
    )
    ...
    original_critique = await original_evaluator.evaluate_content(
        content="Short bad response", context={"confidence": 0.3}
    )
...
async def test_debate_manager_analyst_critic():
    ...
    debate_manager = DebateManager(
        name="test_debate", max_exchanges_per_phase=2, enable_position_evolution=True
    )
    ...
    assert (
        debate_result.topic == "The benefits of artificial intelligence in healthcare"
    )
...
async def test_debate_phases():
    ...
    debate_result = await debate_manager.conduct_analyst_critic_debate(
        topic="Test topic", analyst_agent=agent1, critic_agent=agent2, rounds=1
    )

    # Check phase progression
    opening_exchanges = debate_result.get_phase_exchanges(
...
def test_arbitrator_score_weighting():
    ...
    arbitrator = Arbitrator(
        name="test_arbitrator", default_strategy=ArbitrationStrategy.SCORE_WEIGHTING
    )
...
async def test_arbitrator_expert_judgment():
    ...
    arbitrator = Arbitrator(name="expert_arbitrator", expert_agent=expert)
...
async def test_arbitrator_hybrid():
    ...
    arbitrator = Arbitrator(
        name="hybrid_arbitrator", default_strategy=ArbitrationStrategy.HYBRID
    )
    ...
    debate.exchanges = [
        DebateExchange(
            speaker_id="Agent1", confidence=0.9, content="High confidence response"
        ),
        DebateExchange(
            speaker_id="Agent2", confidence=0.5, content="Lower confidence response"
        ),
    ]
...
async def test_integrated_critique_debate_flow():
    ...
    initial_critique = await evaluator.evaluate_content(
        content=initial_content, context={"confidence": 0.7}
    )
