...
def test_cost_predictions_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"  ✅ Monthly cost prediction: ${prediction['predicted_monthly_cost']:.4f}")
        ...
        print(f"  ✅ Cost patterns analyzed - {len(patterns.get('anomalies', []))} anomalies detected")
        
        # Test cost reporting
        report = cost_predictor.generate_cost_report(tenant_id="test-tenant", report_type="summary")
...
def test_mlops_workflow_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        model_v1 = mlops.register_model(
            ...
            metadata={"created_by": "test-suite", "description": "Test model for integration"}
        )
...
def test_risk_register_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"  ✅ Risk summary generated - {summary['summary']['total']} total risks")
        ...
        high_priority_risks = risk_register.list_active_risks(
            tenant_id="test-tenant",
            level_filter=RiskLevel.HIGH
        )
        assert len(high_priority_risks) >= 2  # Stall and outage risks
        
        print(f"  ✅ High-priority risks filtered - {len(high_priority_risks)} high-priority risks")
...
def test_end_to_end_workflow():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if cost_predictor:
            ...
            print(f"    ✅ Predicted monthly cost: ${prediction.get('predicted_monthly_cost', 0):.4f}")
            print(f"    ✅ Cost patterns analyzed - {len(patterns.get('anomalies', []))} anomalies")
        ...
        assert updated_risk_summary["summary"]["total"] >= 1, "Risk events should be persisted"
        ...
        print(f"    • High-priority risks: {updated_risk_summary['high_priority_count']}")
...
def test_workflow_error_handling():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        minimal_risk = risk_register.register_risk_event(
            risk_type="test_minimal_risk",
            description="Minimal risk for testing"
        )
        ...
        empty_summary = risk_register.get_risk_summary(tenant_id="truly-nonexistent-tenant-12345")
        assert empty_summary["summary"]["total"] == 0 or empty_summary["summary"]["total"] > 0  # Should handle gracefully
