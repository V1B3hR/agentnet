...
def test_dashboard():
    ...
    dashboard.add_violation_event("token_limit", "high", "agent-1", "Token limit", base_time)
...
def test_instrumented_provider():
    ...
    result = provider.infer("Test prompt", agent_name="test-agent", session_id="test-session")
    ...
    print(f"  ✓ Instrumentation data: {instr['duration_ms']:.1f}ms, {instr['provider']}")
