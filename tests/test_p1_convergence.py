...
async def test_convergence_parameters():
    ...
    agents = [
        AgentNet(
            "Alice", {"logic": 0.8, "creativity": 0.6, "analytical": 0.7}, engine=engine
        ),
        AgentNet(
            "Bob", {"logic": 0.6, "creativity": 0.8, "analytical": 0.5}, engine=engine
        ),
    ]
    ...
    print(
        f"  ✅ Strict convergence: {session1['rounds_executed']} rounds, converged: {session1['converged']}"
    )
    ...
    print(
        f"  ✅ Lenient convergence: {session2['rounds_executed']} rounds, converged: {session2['converged']}"
    )
...
async def test_parallel_execution():
    ...
    agents = [
        AgentNet(
            "FastAgent",
            {"logic": 0.7, "creativity": 0.7, "analytical": 0.6},
            engine=engine,
        ),
        AgentNet(
            "SlowAgent",
        ...
        AgentNet(
            "CreativeAgent",
            {"logic": 0.5, "creativity": 0.9, "analytical": 0.4},
            engine=engine,
        ),
    ]
    ...
    print(
        f"  ✅ Rounds: {session['rounds_executed']}, Converged: {session['converged']}"
    )
...
async def test_convergence_strategies():
    ...
    agents = [
        AgentNet(
            "Analyst",
            {"logic": 0.9, "creativity": 0.4, "analytical": 0.9},
            engine=engine,
        ),
        AgentNet(
            "Innovator",
            {"logic": 0.5, "creativity": 0.9, "analytical": 0.6},
            engine=engine,
        ),
    ]
    ...
    for strategy, description in strategies:
        ...
        print(
            f"  ✅ {description}: {session['rounds_executed']} rounds, converged: {session['converged']}"
        )
...
async def test_convergence_edge_cases():
    ...
    single_agent = [
        AgentNet(
            "Solo", {"logic": 0.7, "creativity": 0.7, "analytical": 0.7}, engine=engine
        )
    ]
    ...
    agents = [
        ...
        AgentNet(
            "Persistent2",
            {"logic": 0.3, "creativity": 0.9, "analytical": 0.4},
            engine=engine,
        ),
    ]
    ...
    print(
        f"  ✅ No convergence case: {session_no_conv['rounds_executed']} rounds, converged: {session_no_conv['converged']}"
    )
...
async def run_convergence_experiments():
    ...
    try:
        ...
        print(
            f"Parameter application test: {'✅ PASSED' if param_success else '❌ FAILED'}"
        )

        # Test 2: Parallel execution
        parallel_success = await test_parallel_execution()
        print(
            f"Parallel execution test: {'✅ PASSED' if parallel_success else '❌ FAILED'}"
        )
        ...
        for strategy, result in strategy_results.items():
            print(
                f"  {strategy}: {result['rounds']} rounds, converged: {result['converged']}"
            )
