...
from agentnet.core.orchestration.turn_engine import TurnEngine, TurnMode, TerminationReason
...
async def test_turn_engine_single_agent():
    ...
    assert result.status in [TerminationReason.COMPLETED, TerminationReason.MAX_TURNS_REACHED]
    ...
    for turn in result.turns:
        ...
        assert turn.content.startswith("Initial") or "point" in turn.content or "Final" in turn.content
    ...
    engine = TurnEngine(
        ...
        event_callbacks={
            ...
            "on_turn_end": lambda session_id, agent_name, turn_result:
                event_bus.emit_turn_end(session_id, agent_name, turn_result.to_dict() if hasattr(turn_result, 'to_dict') else {})
        }
    )
    
    # Create agents with varying confidence
    agents = [
        MockAgent("HighConfAgent", ["High quality response."]),  # Will have 0.8 confidence
        MockAgent("LowConfAgent", ["Low quality response."])     # Will have 0.8 confidence (mocked)
    ]
