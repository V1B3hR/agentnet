...
def test_invalid_provider_names():
    ...
    assert "Unsupported vector database provider: invalid_provider" in str(exc_info.value)
...
def test_langchain_compatibility_with_mocked_dependency():
    ...
    with patch.dict('sys.modules', {'langchain': mock_langchain}):
        with patch('agentnet.integrations.langchain.LangChainCompatibilityLayer') as mock_class:
...
def test_integration_module_exports():
    ...
    for export in expected_exports:
        ...
        assert callable(getattr(integrations_module, export)), f"Export not callable: {export}"
