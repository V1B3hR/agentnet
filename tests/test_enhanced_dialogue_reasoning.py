...
try:
    ...
    from agentnet.reasoning.types import (
        ReasoningType, ReasoningEngine, DeductiveReasoning, 
        InductiveReasoning, AbductiveReasoning, AnalogicalReasoning, CausalReasoning
    )
    from agentnet.reasoning.modulation import ReasoningStyleModulator, ReasoningAwareStyleInfluence
    from agentnet.dialogue.enhanced_modes import (
        DialogueMode, OuterDialogue, ModulatedConversation, 
        InterpolationConversation, InnerDialogue, DialogueMapping
    )
...
def test_reasoning_types():
    ...
    test_cases = [
        ("All software bugs are fixable. This is a software bug. Therefore, this bug is fixable.", ReasoningType.DEDUCTIVE),
        ("We observed crashes in versions 1.0, 1.1, and 1.2. Version 1.3 will likely also crash.", ReasoningType.INDUCTIVE),
        ("The system crashed unexpectedly. The most likely explanation is memory corruption.", ReasoningType.ABDUCTIVE),
        ("This network problem is like a traffic jam - we need alternate routes.", ReasoningType.ANALOGICAL),
        ("The database timeout caused the connection failure, which led to the user error.", ReasoningType.CAUSAL)
    ]
    ...
    for task, expected_type in test_cases:
        ...
        print(f"    Selected: {result.reasoning_type.value}, Expected: {expected_type.value}")
        ...
        assert result.reasoning_type == expected_type or specific_result.reasoning_type == expected_type
...
def test_enhanced_dialogue_modes():
    ...
    modulated_result = modulated_dialogue.conduct_dialogue(
        ...
        initial_intensity=0.3, max_intensity=0.8, tension_strategy="gradual_build"
    )
    ...
    inner_result = inner_dialogue.conduct_dialogue(
        [agents[0]], topic, rounds=3,
        reflection_depth="moderate", focus_areas=["reasoning_process", "assumptions"]
    )
...
def test_solution_focused_dialogue():
    ...
    assert result['analysis']['solution_focus_ratio'] > 0.2  # Should have some solution focus
...
def test_dialogue_mapping_visualization():
    ...
    result = mapping_dialogue.conduct_dialogue(
        agents, "Choose the best database technology for our application", 
        rounds=4, mapping_focus="decision_making", track_arguments=True
    )
    ...
    turns = [
        DialogueTurn("Alice", "We need to choose between SQL and NoSQL databases", 0.8, DialogueMode.MAPPING, 1, time.time()),
        DialogueTurn("Bob", "What are our scalability requirements?", 0.7, DialogueMode.MAPPING, 1, time.time()),
        DialogueTurn("Alice", "We expect high read loads but moderate write loads", 0.9, DialogueMode.MAPPING, 2, time.time()),
        DialogueTurn("Bob", "Then I propose we use PostgreSQL with read replicas", 0.8, DialogueMode.MAPPING, 2, time.time())
    ]
    ...
    print(f"  Created map with {len(dialogue_map['nodes'])} nodes and {len(dialogue_map['edges'])} edges")
    
    # Generate visualization data
    vis_data = mapper.generate_visualization_data(dialogue_map['id'])
    
    print(f"  Visualization data: {vis_data['metadata']['node_count']} nodes, {vis_data['metadata']['edge_count']} edges")
    print(f"  Layout type: {vis_data['layout']['type']}")
    print(f"  Legend items: {len(vis_data['legend']['node_types'])}")
    
    # Test export
    with tempfile.TemporaryDirectory() as temp_dir:
        export_path = mapper.export_map(dialogue_map['id'], format="json", 
...
async def test_async_dialogue_modes():
    ...
    print(f"    Max intensity: {async_modulated['analysis']['max_intensity_reached']:.2f}")
...
def test_integration_with_existing_agentnet():
    ...
    agents = [
        AgentNet("IntegrationAgent1", {"logic": 0.8, "creativity": 0.6, "analytical": 0.9}),
        AgentNet("IntegrationAgent2", {"logic": 0.6, "creativity": 0.9, "analytical": 0.7})
    ]
    ...
    multi_result = agents[0].multi_party_dialogue(
        agents, "Test multi-party integration",
        rounds=2, mode="brainstorm"
    )
