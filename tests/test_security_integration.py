...
class TestSecurityIntegration:
    ...
    def setup_method(self):
        ...
        self.auth_middleware = AuthMiddleware(self.rbac_manager, secret_key="test-secret")
    ...
    def test_multi_tenant_isolation_workflow(self):
        ...
        tenant1_admin = self.rbac_manager.create_user(
            "t1_admin", "Tenant1 Admin", "admin@tenant1.com", 
            [Role.ADMIN], "tenant1"
        )
        tenant1_user = self.rbac_manager.create_user(
            "t1_user", "Tenant1 User", "user@tenant1.com",
            [Role.TENANT_USER], "tenant1"
        )
        tenant2_user = self.rbac_manager.create_user(
            "t2_user", "Tenant2 User", "user@tenant2.com",
        ...
        assert "t1_admin_session" in self.isolation_manager._tenant_boundaries["tenant1"]
    ...
    def test_resource_locking_coordination(self):
        ...
        lock2_retry = self.isolation_manager.acquire_resource_lock("session2", resource_id)
    ...
    def test_session_cleanup_releases_resources(self):
        ...
        user = self.rbac_manager.create_user(
            "test_user", "Test User", "test@test.com",
        ...
        locked_count = sum(
            1 for r, s in self.isolation_manager._resource_locks.items() 
            if s == session_id
        )
        ...
        if "test_tenant" in self.isolation_manager._tenant_boundaries:
            assert session_id not in self.isolation_manager._tenant_boundaries["test_tenant"]
    
    def test_authentication_with_isolation(self):
        """Test that authentication and isolation work together."""
        # Create user
        user = self.rbac_manager.create_user(
            "auth_user", "Auth User", "auth@test.com",
            [Role.TENANT_USER], "auth_tenant"
        )
        ...
        assert f"session_{user.user_id}" in self.isolation_manager._tenant_boundaries["auth_tenant"]
    ...
    def test_isolation_stats_accuracy(self):
        ...
        for i in range(3):
            user = self.rbac_manager.create_user(
                f"stats_user_{i}", f"User {i}", f"user{i}@test.com",
                [Role.TENANT_USER], f"tenant_{i % 2}"  # 2 tenants
            )
            users.append(user)
