...
def test_streaming_parser():
    ...
    parser = StreamingParser(
        on_partial_update=on_partial_update, on_complete=on_complete, on_error=on_error
    )
...
async def test_streaming_collaboration():
    ...
    print(
        f"  🔄 Collaborative generation: {len(collab_result['iterations'])} iterations"
    )
...
async def test_collaboration_handlers():
    ...
    collab_handler = CollaborationHandler(
        max_concurrent_streams=3, turn_timeout_seconds=5.0
    )
    ...
    if isinstance(handled_data, dict):
        ...
        print(
            f"  📊 Added collaboration metadata: {handled_data['collaboration_metadata']}"
        )

    # Test error handler
    error_handler = ErrorHandler(max_retries=2, retry_delay=0.1, enable_recovery=True)
    ...
    error_result = await error_handler.handle(
        Exception("Test streaming error"), error_context
    )
    ...
    recovery_result = await error_handler.handle(
        Exception("Recoverable error"), error_context
    )
...
async def test_end_to_end_streaming():
    ...
    for scenario in scenario_data:
        ...
        if response.structured_data:
            print(
                f"  📋 {scenario['agent']}: parsed {len(response.structured_data)} JSON fields"
            )
    ...
    print(
        f"  📡 Session events: {len(partial_events)} partial, {len(complete_events)} complete"
    )
