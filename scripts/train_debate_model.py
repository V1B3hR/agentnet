...
class DebateDatasetProcessor:
        ...
        try:
            ...
            if not potential_files:
                potential_files = list(
                    self.datasets_dir.glob("**/*general*debate*.csv")
                )
            ...
            logger.info(
                f"Loaded UN General Debates: {len(df)} records, columns: {list(df.columns)}"
            )
    ...
    def load_soho_forum_results(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df = pd.read_csv(data_file)
            logger.info(
                f"Loaded SOHO Forum data: {len(df)} records, columns: {list(df.columns)}"
            )
    ...
    def load_un_corpus_1946_2023(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df = pd.read_csv(data_file)
            logger.info(
                f"Loaded UN Corpus 1946-2023: {len(df)} records, columns: {list(df.columns)}"
            )
            ...
            if dataset_name == "un_debates":
                ...
                for _, row in df.iterrows():
                    ...
                    debate_cols = [
                        ...
                        if "debate" in col.lower() or "position" in col.lower()
                    ]
                ...
                for _, row in df.iterrows():
                    text_cols = [
                        col
                        for col in df.columns
                        if "text" in col.lower() or "speech" in col.lower()
                    ]
...
class DebateModelTrainer:
    ...
    def __init__(self, training_data: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized debate model trainer with output dir: {self.training_output_dir}"
        )
    ...
    def run_training_simulation(
        ...
        try:
            ...
            for scenario in scenarios:
                try:
                    ...
                    debate_result = agents[0].debate(
                        agents[1:], scenario["topic"], rounds=3
                    )
                    ...
                    training_results["simulation_results"].append(
                        {
                            ...
                            "debate_result": {
                                ...
                                "final_consensus": debate_result.get(
                                    "final_outcome", "No consensus"
                                ),
        ...
        logger.info(
            f"Training simulation completed: {training_results['training_metrics']}"
        )
        return training_results
    ...
    def save_training_artifacts(self, training_results: Dict[str, Any]) -> None:
            ...
            json.dump(
                {
                    ...
                    "data_statistics": {
                        "total_arguments": len(
                            self.training_data.get("debate_arguments", [])
                        ),
                        "total_positions": len(
                            self.training_data.get("debate_positions", [])
                        ),
                        "total_topics": len(
                            self.training_data.get("debate_topics", [])
                        ),
                    },
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- Total records processed: {self.training_data['metadata']['total_records']}\n"
            )
            f.write(
                f"- Datasets used: {', '.join(self.training_data['metadata']['datasets_used'])}\n"
            )
            f.write(
                f"- Debate arguments extracted: {len(self.training_data.get('debate_arguments', []))}\n"
            )
            f.write(
                f"- Debate positions extracted: {len(self.training_data.get('debate_positions', []))}\n\n"
            )
            f.write("## Training Results\n")
            f.write(
                f"- Scenarios processed: {training_results['scenarios_processed']}\n"
            )
            f.write(
                f"- Successful simulations: {training_results['training_metrics']['successful_simulations']}\n"
            )
            f.write(
                f"- Failed simulations: {training_results['training_metrics']['failed_simulations']}\n\n"
            )
            f.write("## Next Steps\n")
