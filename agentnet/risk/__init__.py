...
class RiskRegister:
    ...
    def register_risk_event(
        ...
        # Create risk event
        risk_id = (
            f"{risk_type}_{int(datetime.now().timestamp())}_{agent_name or 'unknown'}"
        )
        ...
        logger.warning(
            f"Risk event registered: {risk_id} - Level: {risk_event.level.value}"
        )
    ...
    def log_provider_outage_risk(
        self, provider: str, error_rate: float, agent_name: Optional[str] = None
    ) -> RiskEvent:
        """Log a provider outage risk event."""
        level = (
            RiskLevel.CRITICAL
            if error_rate > 0.8
            else RiskLevel.HIGH if error_rate > 0.5 else RiskLevel.MEDIUM
        )
    ...
    def get_risk_summary(
        self, tenant_id: Optional[str] = None, days_back: int = 7
    ) -> Dict[str, Any]:
        ...
        for event in events:
            ...
            risk_counts["by_type"][risk_type] = (
                risk_counts["by_type"].get(risk_type, 0) + 1
            )

        # Get top risks
        top_risks = sorted(
            risk_counts["by_type"].items(), key=lambda x: x[1], reverse=True
        )[:5]
        ...
        return {
            ...
            "summary": risk_counts,
            "top_risks": top_risks,
            "high_priority_count": risk_counts["by_level"]["high"]
            + risk_counts["by_level"]["critical"],
    ...
    def list_active_risks(
        self, tenant_id: Optional[str] = None, level_filter: Optional[RiskLevel] = None
    ) -> List[RiskEvent]:
    ...
    def _initialize_default_risks(self):
        ...
        default_risks = [
            ...
            RiskDefinition(
                ...
                mitigation_actions=[
                    "adjust_policy_thresholds",
                    "provide_override_mechanism",
                ],
            ),
            RiskDefinition(
                risk_type="token_cost_spike",
    ...
    def _save_mitigation(self, mitigation: RiskMitigation):
        ...
        mitigation_file = (
            self.storage_dir / "mitigations" / f"{mitigation.mitigation_id}.json"
        )
        data = {
    ...
    def _load_recent_events(
        self, cutoff_date: datetime, tenant_id: Optional[str] = None
    ) -> List[RiskEvent]:
    ...
    def _trigger_automated_mitigation(
        self, risk_event: RiskEvent, risk_def: RiskDefinition
    ):
    ...
    def _generate_risk_recommendations(self, events: List[RiskEvent]) -> List[str]:
        ...
        high_priority = sum(
            1 for e in events if e.level in [RiskLevel.HIGH, RiskLevel.CRITICAL]
        )

        if high_priority > 5:
            recommendations.append(
                "High number of critical risks - review system configuration"
            )
        ...
        for risk_type, count in risk_types.items():
            if count > 3:
                recommendations.append(
                    f"Recurring {risk_type} risks - implement preventive measures"
                )
