...
from .base import MemoryLayer, MemoryEntry, MemoryRetrieval, MemoryType
...
class MemoryManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.summarize_threshold = self.retrieval_config.get("summarize_threshold", 6000)
    ...
    def store(self, content: str, agent_name: str, metadata: Optional[Dict[str, Any]] = None, tags: Optional[List[str]] = None) -> bool:
        ...
        if MemoryType.EPISODIC in self.layers:
            should_store_episodic = (
                tags or 
                metadata and metadata.get("important", False) or
                len(content) > 200  # Store longer content in episodic
            )
    ...
    def retrieve(self, query: str, agent_name: Optional[str] = None, context: Optional[Dict[str, Any]] = None) -> MemoryRetrieval:
        ...
        if MemoryType.SHORT_TERM in self.layers:
            short_term_entries = self.layers[MemoryType.SHORT_TERM].retrieve(
                query, 
                max_entries=self.retrieval_config.get("short_term_count", 10)
            )
    ...
    def _extract_tags_from_query(self, query: str, context: Optional[Dict[str, Any]] = None) -> List[str]:
        """Extract potential tags from query and context."""
        tags = []
        
        # Simple keyword extraction
        keywords = ["error", "bug", "issue", "solution", "important", "critical", "urgent"]
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "enabled_layers": list(self.layers.keys()),
            "layers": {}
        }
