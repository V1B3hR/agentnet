...
class RetentionPolicy(ABC):
    ...
    def _calculate_importance_score(self, entry: MemoryEntry) -> float:
        ...
        important_keywords = {
            "error", "critical", "fail", "success", "decision", "conclusion",
            "result", "summary", "important", "warning", "alert"
        }
...
class LRURetentionPolicy(RetentionPolicy):
    ...
    def select_for_eviction(
        ...
        entry_ids.sort(key=lambda eid: self.metrics.get(eid, RetentionMetrics()).last_accessed)
...
class LFURetentionPolicy(RetentionPolicy):
    ...
    def select_for_eviction(
        ...
        entry_ids = list(entries.keys())
        entry_ids.sort(key=lambda eid: self.metrics.get(eid, RetentionMetrics()).access_count)
...
class SemanticSalienceRetentionPolicy(RetentionPolicy):
    ...
    def should_retain(self, entry: MemoryEntry, entry_id: str) -> bool:
        ...
        metrics = self.metrics[entry_id]
        return (metrics.semantic_score >= self.min_semantic_score or 
                metrics.importance_score >= self.min_importance_score)
    ...
    def select_for_eviction(
        ...
        def combined_score(entry_id: str) -> float:
            ...
            return (metrics.semantic_score * 0.6 + 
                   metrics.importance_score * 0.4)
...
class RetentionManager:
    ...
    def get_retention_stats(self) -> Dict[str, Any]:
        ...
        for memory_type, policy in self.policies.items():
            ...
            if total_entries > 0:
                avg_access_count = sum(m.access_count for m in policy.metrics.values()) / total_entries
                avg_age = sum(m.age_seconds() for m in policy.metrics.values()) / total_entries
                avg_semantic_score = sum(m.semantic_score for m in policy.metrics.values()) / total_entries
            else:
