...
class ProblemSolutionTransition:
    ...
    def identify_transition_opportunity(
        self, dialogue_content: str, current_phase: ConversationPhase
    ) -> Optional[str]:
        """
        ...
        has_problem_focus = any(
            indicator in content_lower for indicator in problem_indicators
        )
        shows_solution_readiness = any(
            indicator in content_lower for indicator in solution_readiness_indicators
        )
        ...
        if has_problem_focus and shows_solution_readiness:
            ...
            return "scaling"
        elif "when" in content_lower and (
            "better" in content_lower or "worked" in content_lower
        ):
            return "exception"
    ...
    def create_transition_prompt(self, pattern: str, problem_context: str) -> str:
        ...
        """
        base_transition = self.transition_patterns.get(
            pattern, self.transition_patterns["direct"]
        )
...
class ConstructiveQuestioning:
    ...
    def generate_question(
        self, question_type: QuestionType, context: Dict[str, Any] = None
    ) -> str:
        """
        ...
        if context and question_type == QuestionType.SCALING:
            ...
            template = template.replace("{current}", str(current)).replace(
                "{target}", str(target)
            )

        return template

    def identify_question_opportunity(
        self, dialogue_content: str, conversation_phase: ConversationPhase
    ) -> QuestionType:
        """
...
class SolutionFocusedDialogue:
    ...
    def conduct_solution_focused_dialogue(
        self,
        ...
        **kwargs,
            ...
            for agent in agents:
                ...
                if transition_strategy == "adaptive":
                    transition_opportunity = (
                        self.transition_manager.identify_transition_opportunity(
                            turn["content"], current_phase
                        )
                    )
                    if (
                        transition_opportunity and len(transcript) > 2
                    ):  # Allow some exploration first
        ...
        return {
            ...
            "problems_identified": [
                self._problem_to_dict(p) for p in problems_identified
            ],
            "solutions_proposed": [
                self._solution_to_dict(s) for s in solutions_proposed
            ],
...
"""
    ...
    def _extract_solution_elements(
        self,
        turn: Dict[str, Any],
    ...
    def _analyze_solution_focused_dialogue(
        self,
        transcript: List[Dict[str, Any]],
        problems: List[ProblemStatement],
        solutions: List[SolutionElement],
        questions: List[Dict[str, Any]],
        phase_history: List[ConversationPhase],
    ) -> Dict[str, Any]:
        ...
        solution_focused_turns = sum(
            1
            for turn in transcript
            if any(
                word in turn["content"].lower()
                for word in ["solution", "approach", "way", "plan", "strategy"]
            )
        )
        solution_focus_ratio = (
            solution_focused_turns / total_turns if total_turns > 0 else 0
        )
        ...
        problem_turns = sum(
            1
            for turn in transcript
            if turn["phase"] == ConversationPhase.PROBLEM_IDENTIFICATION.value
        )
        solution_turns = sum(
            1
            for turn in transcript
        ...
        transition_effectiveness = (
            "high"
            if solution_turns > problem_turns
            else "moderate" if solution_turns == problem_turns else "low"
        )
        ...
        effectiveness_score = (
            ...
            + (
                1.0
                if transition_effectiveness == "high"
                else 0.5 if transition_effectiveness == "moderate" else 0.0
            )
            * 10
        )
        ...
        overall_effectiveness = (
            ...
            else "moderate" if effectiveness_score > 40 else "needs_improvement"
        )
