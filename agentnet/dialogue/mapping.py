...
class DialogueMapper:
    ...
    def create_dialogue_map(
        ...
        topic_node = MapNode(
            ...
            round_number=0,
            timestamp=(
                dialogue_state.transcript[0].timestamp if transcript else time.time()
            ),
            confidence=1.0,
        ...
        for turn in transcript:
            ...
            specialized_nodes, specialized_edges = self._extract_specialized_nodes(
                turn, turn_node
            )
    ...
    def _create_turn_node(self, turn: "DialogueTurn", node_id: str) -> MapNode:
        ...
        if "?" in turn.content:
            ...
            for word in ["solution", "approach", "plan", "strategy"]
        ):
            node_type = NodeType.SOLUTION
        elif any(word in content_lower for word in ["decision", "choose", "decide"]):
            node_type = NodeType.DECISION
        elif any(
            word in content_lower for word in ["argue", "disagree", "oppose", "support"]
        ):
    ...
    def _determine_edge_type(
        self, turn: "DialogueTurn", existing_nodes: List[MapNode]
    ) -> EdgeType:
        ...
        if any(
            word in content_lower for word in ["agree", "support", "build on", "yes"]
        ):
            return EdgeType.SUPPORTS
        elif any(
            word in content_lower for word in ["disagree", "oppose", "however", "but"]
        ):
    ...
    def _extract_specialized_nodes(
        self, turn: "DialogueTurn", parent_node: MapNode
        ...
        if any(
            word in content_lower
            for word in ["constraint", "limitation", "cannot", "must not"]
        ):
            ...
            constraint_node = MapNode(
                ...
                timestamp=turn.timestamp,
                confidence=turn.confidence
                * 0.8,  # Slightly lower confidence for extracted elements
                content=turn.content,
        ...
        if any(
            word in content_lower
            for word in ["criteria", "requirement", "important", "priority"]
        ):
    ...
    def _identify_cross_turn_relationships(
        self, nodes: List[MapNode], transcript: List["DialogueTurn"]
    ) -> List[MapEdge]:
        ...
        for question in questions:
            ...
            for statement in statements:
                if (
                    statement.round_number > question.round_number
                    and statement.agent != question.agent
                    and statement.round_number <= question.round_number + 2
                ):  # Within 2 rounds
    ...
    def generate_visualization_data(
        self, map_id: str, config: Optional[VisualizationConfig] = None
    ) -> Dict[str, Any]:
        """
        ...
        visualization_data = {
            ...
            "nodes": vis_nodes,
            "edges": vis_edges,
            "layout": {"type": config.layout, "config": layout_config},
            "legend": legend,
            "metadata": {
    ...
    def _prepare_node_for_visualization(
        self, node_data: Dict[str, Any], config: VisualizationConfig
    ) -> Dict[str, Any]:
        ...
        vis_node["metadata"] = {
            "content_preview": (
                node_data["content"][:100] + "..."
                if len(node_data["content"]) > 100
    ...
    def _prepare_edge_for_visualization(
        self, edge_data: Dict[str, Any], config: VisualizationConfig
    ) -> Dict[str, Any]:
    ...
    def _generate_layout_config(
        self, dialogue_map: Dict[str, Any], config: VisualizationConfig
    ) -> Dict[str, Any]:
        ...
        layout_configs = {
            ...
            "circular": {"radius": 200, "start_angle": 0, "clockwise": True},
        }

        base_config = layout_configs.get(
            config.layout, layout_configs["force_directed"]
        )

        # Add grouping configuration
        if config.group_by_agent:
            base_config["clustering"] = {"enabled": True, "cluster_by": "agent"}

        return base_config

    def _create_visualization_legend(
    ...
    def export_map(
        self, map_id: str, format: str = "json", file_path: Optional[str] = None
    ) -> str:
        """
...
"""
    ...
    def _export_to_dot(self, dialogue_map: Dict[str, Any]) -> str:
        ...
        # Add nodes
        for node in dialogue_map["nodes"]:
            node_attrs = [f'label="{node["label"]}"', f'shape="box"', f'style="filled"']
