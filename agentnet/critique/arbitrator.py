...
class Arbitrator:
    ...
    def __init__(
        ...
        logger.info(f"Arbitrator '{name}' initialized with strategy {default_strategy.value}")
    ...
    async def arbitrate_debate(
        ...
        try:
            ...
            logger.info(f"Arbitration completed: {result.winning_agent} wins with {result.confidence:.2f} confidence")
    ...
    async def _score_weighting_arbitration(
        ...
        for position_id, position in debate.positions.items():
            ...
            completeness_score = min(1.0, len(position.supporting_arguments) / 5.0)  # Max at 5 args
        ...
        if position_scores:
            ...
            # Build reasoning
            score_details = [f"{pos_id}: {score:.2f}" for pos_id, score in position_scores.items()]
    ...
    async def _majority_vote_arbitration(
        ...
        for exchange in debate.exchanges:
            agent_exchange_counts[exchange.speaker_id] = agent_exchange_counts.get(exchange.speaker_id, 0) + 1
        ...
        for exchange in debate.exchanges:
            ...
            agreement_count = sum(1 for pattern in agreement_patterns if pattern in content_lower)
            disagreement_count = sum(1 for pattern in disagreement_patterns if pattern in content_lower)
    ...
    async def _evidence_based_arbitration(
        ...
        for position in debate.positions.values():
            ...
            agent_evidence_scores[agent_id] = agent_evidence_scores.get(agent_id, 0) + evidence_score
    ...
    async def _hybrid_arbitration(
        ...
        # Confidence based
        confidence_result = ArbitrationResult(strategy=ArbitrationStrategy.CONFIDENCE_BASED)
        ...
        for strategy_name, strategy_result in strategies_results.items():
            if strategy_result.winning_agent:
                agent_votes[strategy_result.winning_agent] = agent_votes.get(strategy_result.winning_agent, 0) + 1
    ...
    def _build_expert_judgment_prompt(self, debate: DebateResult) -> str:
        ...
        if debate.positions:
            ...
            for pos_id, position in debate.positions.items():
                summary_parts.append(f"- {position.agent_id}: {position.statement[:200]}...")
        
        # Add recent exchanges
        recent_exchanges = debate.exchanges[-4:] if len(debate.exchanges) > 4 else debate.exchanges
        if recent_exchanges:
            summary_parts.append("Recent exchanges:")
            for exchange in recent_exchanges:
                summary_parts.append(f"- {exchange.speaker_id}: {exchange.content[:100]}...")
    ...
    async def _parse_expert_judgment(
        ...
        for agent_name in agent_names:
            if f"winner: {agent_name}" in content_lower or f"{agent_name} wins" in content_lower:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        consensus_rate = (
            ...
            if self.arbitrations_performed > 0 else 0.0
        )
