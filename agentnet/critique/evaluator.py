...
class CritiqueEvaluator:
    ...
    async def evaluate_content(
        ...
        try:
            ...
            total_score = (self.average_quality_score * (self.critiques_performed - 1) + 
                          critique.overall_score) / self.critiques_performed
            self.average_quality_score = total_score
            
            logger.debug(f"Critique completed: {critique.overall_score:.2f} score, revision={critique.needs_revision}")
    ...
    async def _automated_scoring(
        ...
        critique.truthiness_score = await self._calculate_truthiness_score(content, context)
        
        # Coherence score - logical flow and consistency
        critique.coherence_score = await self._calculate_coherence_score(content, context)
        
        # Completeness score - addresses the prompt/question
        critique.completeness_score = await self._calculate_completeness_score(content, context)
    ...
    async def _calculate_quality_score(self, content: str, context: Dict[str, Any]) -> float:
    ...
    async def _calculate_truthiness_score(self, content: str, context: Dict[str, Any]) -> float:
        ...
        if any(marker in content for marker in ["[", "]", "according to", "research shows"]):
            score += 0.2
        
        return min(1.0, max(0.0, score))
    
    async def _calculate_coherence_score(self, content: str, context: Dict[str, Any]) -> float:
        """Calculate logical coherence score."""
        score = 0.6  # Base score
        
        # Check for logical connectors
        connectors = ["therefore", "however", "furthermore", "consequently", "because", "since"]
    ...
    async def _calculate_completeness_score(self, content: str, context: Dict[str, Any]) -> float:
    ...
    async def _agent_critique(
        ...
        try:
            ...
            if hasattr(self.critique_agent, 'async_generate_reasoning_tree'):
                response = await self.critique_agent.async_generate_reasoning_tree(critique_prompt)
            else:
    ...
    async def _heuristic_critique(
        ...
        if critique.truthiness_score > 0.7:
            critique.strengths.append("Appears factually sound with appropriate caveats")
        
        # Analyze weaknesses
        if critique.quality_score < 0.5:
            critique.weaknesses.append("Response lacks depth and detail")
            critique.suggestions.append("Provide more comprehensive analysis and examples")
        ...
        if critique.coherence_score < 0.6:
            ...
            critique.suggestions.append("Use clearer transitions and logical connectors")
        
        if critique.completeness_score < 0.6:
            critique.weaknesses.append("Does not fully address the prompt")
            critique.suggestions.append("Ensure all aspects of the question are covered")
        ...
        if critique.weaknesses:
            feedback_parts.append(f"Areas for improvement: {'; '.join(critique.weaknesses)}")
        
        if critique.suggestions:
            feedback_parts.append(f"Suggestions: {'; '.join(critique.suggestions)}")
        
        critique.critique_text = " | ".join(feedback_parts) if feedback_parts else "No specific feedback."
    ...
    async def _parse_agent_critique(self, critique: CritiqueResult, critique_text: str) -> None:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        revision_rate = self.revisions_triggered / self.critiques_performed if self.critiques_performed > 0 else 0.0
...
class RevisionEvaluator:
    ...
    async def evaluate_revision(
        ...
        logger.debug(f"Revision evaluation: {improvement:.2f} improvement, successful={revision_successful}")
        
        return revision_successful, new_critique
    
    def get_stats(self) -> Dict[str, Any]:
        """Get revision evaluator statistics."""
        success_rate = self.successful_revisions / self.revisions_evaluated if self.revisions_evaluated > 0 else 0.0
