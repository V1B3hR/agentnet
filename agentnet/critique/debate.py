...
class DebateResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "topic": self.topic,
            "participants": {agent_id: role.value for agent_id, role in self.participants.items()},
            "start_time": self.start_time,
            "end_time": self.end_time,
            "duration": self.duration,
            "current_phase": self.current_phase.value,
            "completed_phases": [p.value for p in self.completed_phases],
            "positions": {pos_id: pos.to_dict() for pos_id, pos in self.positions.items()},
...
class DebateManager:
    ...
    async def conduct_analyst_critic_debate(
        ...
        try:
            ...
            await self._conduct_opening_statements(debate, [analyst_agent, critic_agent])
            ...
            await self._conduct_closing_statements(debate, [analyst_agent, critic_agent])
    ...
    async def _conduct_opening_statements(
        ...
        for agent in agents:
            ...
            if role in [DebateRole.ANALYST, DebateRole.PROPONENT]:
                position = await self._extract_position(agent.name, role, exchange.content)
                position.stance = "for" if role == DebateRole.PROPONENT else "analytical"
    ...
    async def _conduct_argumentation_round(
        ...
        logger.debug(f"Completed argumentation round {round_num} for debate {debate.debate_id}")
    ...
    async def _conduct_cross_examination(
        ...
        question_prompt = self._build_cross_examination_prompt(
            debate.topic, DebateRole.CRITIC, debate.exchanges, target_role=DebateRole.ANALYST
        )
    ...
    async def _conduct_closing_statements(
        ...
        for agent in agents:
            ...
            prompt = self._build_closing_statement_prompt(debate.topic, role, debate.exchanges)
    ...
    def _build_opening_statement_prompt(self, topic: str, role: DebateRole) -> str:
        ...
        if role == DebateRole.ANALYST:
            return (f"As an analyst, provide your opening analysis of: {topic}. "
            ...
            return (f"As a critic, provide your opening critical perspective on: {topic}. "
    ...
    def _build_argumentation_prompt(
        self, topic: str, role: DebateRole, exchanges: List[DebateExchange], round_num: int
    ) -> str:
        ...
        context = "\n".join([f"{ex.speaker_id}: {ex.content[:200]}..." for ex in recent_exchanges])
        
        if role == DebateRole.ANALYST:
            return (f"Round {round_num} - Continue your analysis of: {topic}.\n\n"
            ...
            return (f"Round {round_num} - Continue your critique of: {topic}.\n\n"
                   f"Previous discussion:\n{context}\n\n"
    ...
    def _build_cross_examination_prompt(
        self, topic: str, role: DebateRole, exchanges: List[DebateExchange], target_role: DebateRole
    ) -> str:
        """Build cross-examination prompt."""
        # Get target's recent statements
        target_exchanges = [ex for ex in exchanges if ex.speaker_id != exchanges[-1].speaker_id][-2:]
        target_context = "\n".join([f"{ex.content[:200]}..." for ex in target_exchanges])
        
        return (f"Cross-examination phase for topic: {topic}.\n\n"
    ...
    def _build_response_prompt(self, topic: str, role: DebateRole, question: str) -> str:
        """Build response prompt for cross-examination."""
        return (f"You are being cross-examined about: {topic}.\n\n"
               f"Question: {question}\n\n"
    ...
    def _build_closing_statement_prompt(
        ...
        return (f"Provide your closing statement for the debate on: {topic}.\n\n"
    ...
    async def _update_positions_from_exchange(
        ...
        agent_positions = [pos for pos in debate.positions.values() 
    ...
    async def _analyze_debate_outcome(self, debate: DebateResult) -> None:
        ...
        analyst_exchanges = [ex for ex in debate.exchanges if ex.speaker_id in 
                           [k for k, v in debate.participants.items() if v == DebateRole.ANALYST]]
        critic_exchanges = [ex for ex in debate.exchanges if ex.speaker_id in
                          [k for k, v in debate.participants.items() if v == DebateRole.CRITIC]]
        
        # Calculate average confidence scores
        analyst_avg_confidence = (
            sum(ex.confidence for ex in analyst_exchanges) / len(analyst_exchanges)
            if analyst_exchanges else 0.0
        )
        
        critic_avg_confidence = (
            sum(ex.confidence for ex in critic_exchanges) / len(critic_exchanges)
            if critic_exchanges else 0.0
        )
        ...
        if confidence_diff < 0.1:
            ...
            debate.final_synthesis = "Consensus reached through balanced analysis and critique"
    ...
    async def _generate_final_synthesis(self, debate: DebateResult) -> str:
        ...
        closing_exchanges = [ex for ex in debate.exchanges 
                           if ex.phase == DebatePhase.CLOSING_STATEMENTS]
        ...
        if key_points:
            ...
            return "Debate completed with structured argumentation and cross-examination."
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        consensus_rate = (
            ...
            if self.debates_conducted > 0 else 0.0
        )
