...
class LanguageDetector:
    ...
    LANGUAGE_PATTERNS = {
        ...
        SupportedLanguage.RUSSIAN: [
            r'[а-яё]+', r'\b(и|в|на|с|по|что|как)\b'
        ],
...
class MultiLingualPolicyTranslator:
    ...
    def check_content_safety(
        ...
        if violations:
            logger.warning(f"Detected {len(violations)} policy violations in {detected_language} content")
    ...
    def get_violation_statistics(
        ...
        return {
            ...
            "most_common_language": max(by_language.items(), key=lambda x: x[1])[0].value if by_language else None,
            "most_common_violation": max(by_type.items(), key=lambda x: x[1])[0].value if by_type else None
        }
    ...
    def export_rules_config(self, filepath: str) -> None:
        ...
        for rule_id, rule in self.safety_rules.items():
            rule_config = {
                ...
                "action": rule.action,
                "descriptions": {lang.value: desc for lang, desc in rule.description.items()},
                "patterns": {lang.value: patterns for lang, patterns in rule.patterns.items()},
                "keywords": {lang.value: keywords for lang, keywords in rule.keywords.items()},
    ...
    def _check_rule_against_content(
        ...
        patterns = rule.patterns.get(detected_language) or rule.patterns.get(SupportedLanguage.ENGLISH, [])
        keywords = rule.keywords.get(detected_language) or rule.keywords.get(SupportedLanguage.ENGLISH, [])
