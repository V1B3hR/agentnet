...
class MetaController:
    ...
    def update_performance(self, agent_id: str, performance_score: float) -> None:
        ...
        if agent_id in self.agents:
            ...
            logger.debug(
                f"Updated performance for agent {agent_id}: {performance_score}"
            )
    ...
    def analyze_performance(self) -> Dict[str, Any]:
        ...
        if len(underperforming) > total_agents * 0.3:
            analysis["suggestions"].append(
                "Consider role reassignment for underperforming agents"
            )
    ...
    def auto_reconfigure(
        self, trigger: ReconfigurationTrigger, context: Dict[str, Any]
    ) -> bool:
        ...
        if (
            ...
            and (datetime.now() - self.last_reconfiguration).total_seconds()
            < self.reconfiguration_cooldown
        ):
        ...
        if trigger == ReconfigurationTrigger.PERFORMANCE_THRESHOLD:
            ...
            if len(analysis["underperforming_agents"]) > 0 and len(self.agents) > 2:
                for agent_id in analysis["underperforming_agents"][
                    :1
            ...
            # Add specialist if average performance is low
                ...
                analysis["average_performance"] < self.performance_threshold
            ...
            if complexity > 0.7 and len(self.agents) < self.max_agents:
                ...
                for agent_id in list(self.agents.keys())[
                    :-1
                ]:  # Exclude the coordinator itself
    ...
    def get_graph_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_reconfiguration": (
                self.last_reconfiguration.isoformat()
                if self.last_reconfiguration
                else None
            ),
        }
