...
class PerformanceFeedbackCollector:
    ...
    def __init__(
        ...
        self.strategy_profiles: Dict[OrchestrationStrategy, StrategyPerformanceProfile] = {}
        
        # Callbacks
        self.on_performance_recorded: Optional[Callable[[PerformanceSnapshot], None]] = None
        self.on_strategy_adapted: Optional[Callable[[OrchestrationStrategy, OrchestrationStrategy], None]] = None
    ...
    def get_strategy_recommendation(
        ...
        for strategy, profile in self.strategy_profiles.items():
            ...
            score = self._calculate_strategy_score(
                profile, context, objective, task_complexity, agent_count,
                latency_constraint, accuracy_requirement
            )
    ...
    def analyze_performance_trends(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        # Overall trends
        success_rate = sum(1 for snap in recent_snapshots if snap.success) / len(recent_snapshots)
        
        # Metric trends
        metric_trends = {}
        for metric in PerformanceMetric:
            values = [
                snap.metrics.get(metric, 0.0) for snap in recent_snapshots
        ...
        for strategy in OrchestrationStrategy:
            strategy_snapshots = [snap for snap in recent_snapshots if snap.strategy == strategy]
            if strategy_snapshots:
                strategy_success_rate = sum(1 for snap in strategy_snapshots if snap.success) / len(strategy_snapshots)
                strategy_comparison[strategy] = {
                    ...
                    ]) if any(PerformanceMetric.LATENCY in snap.metrics for snap in strategy_snapshots) else None
                }
    ...
    def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        # Short-term trends
        short_term_success_rate = sum(1 for snap in recent_snapshots if snap.success) / len(recent_snapshots)
    ...
    def optimize_orchestration(
        ...
        recommended_strategy, confidence = self.get_strategy_recommendation(context, objective)
                ...
                if metric in current_profile.avg_metrics:
                    ...
                    if current_value < avg_value * 0.8:  # 20% below average
                        optimization_result["recommended_changes"].append({
                            ...
                            "target": avg_value,
                            "suggestion": self._get_improvement_suggestion(metric, current_value, avg_value)
                        })
    ...
    def _update_strategy_profile(self, snapshot: PerformanceSnapshot) -> None:
        ...
        if strategy not in self.strategy_profiles:
            self.strategy_profiles[strategy] = StrategyPerformanceProfile(strategy=strategy)
        ...
        profile.success_rate = (
            (profile.success_rate * (profile.total_runs - 1) + (1 if snapshot.success else 0))
            / profile.total_runs
        )
    ...
    def _calculate_strategy_score(
        ...
        if objective == OptimizationObjective.MINIMIZE_LATENCY:
            ...
            cost_efficiency = profile.avg_metrics.get(PerformanceMetric.COST_EFFICIENCY, 0.5)
            ...
            latency_score = 1.0 - min(1.0, profile.avg_metrics.get(PerformanceMetric.LATENCY, 1.0) / 10.0)
            error_rate = profile.avg_metrics.get(PerformanceMetric.ERROR_RATE, 0.5)
            
            base_score = (base_score + accuracy + latency_score + (1.0 - error_rate)) / 4.0
        
        # Context adjustments
        if task_complexity > 0.7 and profile.strategy in [OrchestrationStrategy.DEBATE, OrchestrationStrategy.CONSENSUS]:
    ...
    def _fallback_strategy_selection(self, context: Dict[str, Any]) -> OrchestrationStrategy:
    ...
    def _generate_performance_recommendations(self, snapshots: List[PerformanceSnapshot]) -> List[str]:
        ...
        if success_rate < 0.8:
            recommendations.append("Low success rate detected - consider strategy adjustment")
        ...
        if latency_values and statistics.mean(latency_values) > 5.0:
            recommendations.append("High latency detected - consider parallel processing")
    ...
    def _generate_performance_alerts(self, recent_snapshots: List[PerformanceSnapshot]) -> List[Dict[str, Any]]:
    ...
    def _get_improvement_suggestion(self, metric: PerformanceMetric, current: float, target: float) -> str:
...
class AdaptiveOrchestrator:
    ...
    def should_adapt_strategy(
        ...
        # Get recommendation
        recommended_strategy, confidence = self.feedback_collector.get_strategy_recommendation(
    ...
    def adapt_strategy(
        ...
        logger.info(f"Strategy adapted from {old_strategy} to {recommended_strategy} (confidence: {confidence:.3f})")
    ...
    def get_orchestration_config(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommended_parameters": self._get_strategy_parameters(self.current_strategy, context)
        }
    
    def _on_strategy_adapted(self, old_strategy: OrchestrationStrategy, new_strategy: OrchestrationStrategy) -> None:
        """Callback when strategy is adapted."""
        
        logger.info(f"Strategy adaptation callback: {old_strategy} -> {new_strategy}")
    
    def _get_strategy_parameters(self, strategy: OrchestrationStrategy, context: Dict[str, Any]) -> Dict[str, Any]:
