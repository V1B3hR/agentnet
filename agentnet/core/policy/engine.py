...
class PolicyEngine:
    ...
    def evaluate(
        ...
        result = PolicyResult(
            action=action,
            passed=(action in [PolicyAction.ALLOW, PolicyAction.LOG, PolicyAction.TRANSFORM]),
    ...
    def _determine_action(
        ...
        if self.strict_mode:
            return PolicyAction.BLOCK, f"Strict mode: {len(violations)} violations detected"
        
        # Check if we exceed max violations
        if len(violations) > self.max_violations:
            return PolicyAction.BLOCK, f"Too many violations: {len(violations)} > {self.max_violations}"
        
        # Check severity levels
        severe_violations = [v for v in violations if v.severity == Severity.SEVERE]
        if severe_violations:
            return PolicyAction.BLOCK, f"Severe violations: {[v.rule_name for v in severe_violations]}"
        
        major_violations = [v for v in violations if v.severity == Severity.MAJOR]
        if len(major_violations) >= 2:
            return PolicyAction.BLOCK, f"Multiple major violations: {[v.rule_name for v in major_violations]}"
        
        # Check if content can be transformed
        if self.enable_transformations and self._can_transform(violations, context):
            return PolicyAction.TRANSFORM, f"Content can be transformed to fix {len(violations)} violations"
        
        # Major violations might require approval
        if major_violations:
            return PolicyAction.REQUIRE_APPROVAL, f"Major violations require approval: {[v.rule_name for v in major_violations]}"
        
        # Minor violations can be logged
        return PolicyAction.LOG, f"Minor violations logged: {[v.rule_name for v in violations]}"
    
    def _can_transform(self, violations: List[RuleResult], context: Dict[str, Any]) -> bool:
    ...
    def _transform_keywords(self, content: str, violation: RuleResult) -> str:
        ...
        if violation.rationale and "Found prohibited keywords:" in violation.rationale:
            ...
            for keyword in keywords:
                ...
                transformed = re.sub(pattern, "[REDACTED]", transformed, flags=re.IGNORECASE)
