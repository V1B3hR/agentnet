...
class SuccessCriteria:
    ...
    def __post_init__(self):
        ...
        if self.type == CriteriaType.SEMANTIC_SCORE and not (
            self.reference_text or self.reference_id
        ):
            raise ValueError(
                "semantic_score criteria must specify reference_text or reference_id"
            )
...
class MetricsCalculator:
    ...
    def evaluate_content(
        ...
        metrics = EvaluationMetrics(
            scenario_name=scenario_name, metadata=metadata or {}
        )
        ...
        for criteria_obj in criteria:
            try:
                ...
                metrics.add_criteria_result(result)
                self.logger.error(
                    f"Error evaluating criteria {criteria_obj.type}: {str(e)}"
                )
    ...
    def _evaluate_single_criteria(
        ...
        if criteria.type == CriteriaType.KEYWORD_PRESENCE:
            ...
            return self._evaluate_confidence_threshold(
                content, criteria, criteria_name, metadata
            )
        elif criteria.type == CriteriaType.RULE_VIOLATIONS_COUNT:
            return self._evaluate_violations_count(
                content, criteria, criteria_name, metadata
            )
    ...
    def _evaluate_keyword_presence(
        self, content: str, criteria: SuccessCriteria, criteria_name: str
    ) -> CriteriaResult:
    ...
    def _evaluate_semantic_score(
        ...
        if criteria.reference_text:
            ...
            if not reference_text:
                raise ValueError(
                    f"Reference text not found for ID: {criteria.reference_id}"
                )
        else:
    ...
    def _evaluate_violations_count(
        ...
        if criteria.max_violations is not None and criteria.max_violations > 0:
            score = max(
                0.0,
                (criteria.max_violations - violations_count) / criteria.max_violations,
            )
        else:
    ...
    def _evaluate_custom_function(
        ...
        if criteria.function_name not in self.custom_functions:
            raise ValueError(
                f"Custom function not registered: {criteria.function_name}"
            )
        ...
        try:
            ...
            raise ValueError(
                f"Custom function {criteria.function_name} failed: {str(e)}"
            )
    ...
    def _calculate_standard_metrics(
        ...
        metrics.novelty_score = (
            len(novel_words) / len(unique_words) if unique_words else 0.0
        )
...
if __name__ == "__main__":
    ...
    # Example metadata
    metadata = {"confidence": 0.85, "violations": []}

    calculator = MetricsCalculator()
    results = calculator.evaluate_content(
        content, criteria, "resilience_test", metadata
    )
    ...
    for result in results.criteria_results:
        print(
            f"  {result.criteria_name}: {'PASS' if result.passed else 'FAIL'} ({result.score:.2f})"
        )
