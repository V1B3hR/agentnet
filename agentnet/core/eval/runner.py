...
from .metrics import CriteriaType, EvaluationMetrics, MetricsCalculator, SuccessCriteria
...
class EvaluationRunner:
    ...
    def set_dialogue_executor(
        self, executor: Callable[[List[str], str, Dict[str, Any]], Dict[str, Any]]
    ):
        """
    ...
    def set_workflow_executor(
        self, executor: Callable[[Dict[str, Any], Dict[str, Any]], Dict[str, Any]]
    ):
        """
    ...
    async def run_scenario(
        self, scenario: EvaluationScenario, context: Optional[Dict[str, Any]] = None
    ) -> ScenarioResult:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Scenario '{scenario.name}' completed with score {result.metrics.success_rate:.2f}"
            )
    ...
    async def run_suite(
        ...
        suite_result = SuiteResult(
            suite_name=suite.name, execution_id=execution_id, start_time=time.time()
        )

        self.logger.info(
            f"Running evaluation suite '{suite.name}' with {len(suite.scenarios)} scenarios"
        )
        ...
        try:
            if parallel and len(suite.scenarios) > 1:
                ...
                tasks = [
                    self.run_scenario(scenario, context) for scenario in suite.scenarios
                ]
            ...
            self.logger.info(
                f"Suite '{suite.name}' completed: {suite_result.summary.get('success_rate', 0):.2f} success rate"
            )
    ...
    async def _execute_dialogue_scenario(
        self, scenario: EvaluationScenario, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if scenario.timeout:
            ...
            return await self._call_dialogue_executor(
                scenario.agents, scenario.topic, config
            )
    ...
    async def _execute_workflow_scenario(
        ...
        config = {"timeout": scenario.timeout, **scenario.metadata, **context}
    ...
    async def _call_dialogue_executor(
        self, agents: List[str], topic: str, config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _call_workflow_executor(
        self, task_graph: Dict[str, Any], config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _extract_content_for_evaluation(
        self, session_data: Dict[str, Any], scenario: EvaluationScenario
    ) -> str:
