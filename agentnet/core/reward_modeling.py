...
class RewardModel:
    ...
    def predict_reward(self, agent_id: str, action_context: Dict[str, Any]) -> float:
        ...
        if agent_id in self.reward_history and self.reward_history[agent_id]:
            return statistics.mean(
                self.reward_history[agent_id][-10:]
            )  # Last 10 scores
    ...
    def create_evaluation_batch(self, days_back: int = 1) -> EvaluationBatch:
        ...
        if recent_feedback:
            ...
            batch.evaluation_metrics = {
                ...
                "positive_feedback_ratio": len([s for s in scores if s > 0])
                / len(scores),
                "agent_coverage": len(
                    set(feedback.agent_id for feedback in recent_feedback)
                ),
            }
        ...
        logger.info(
            f"Created evaluation batch {batch.id} with {len(recent_feedback)} entries"
        )
    ...
    def get_learning_insights(self, agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if agent_id:
            ...
            return {
                ...
                "consistency": 1.0
                - (statistics.stdev(recent_scores) if len(recent_scores) > 1 else 0.0),
            }
        else:
            # Global insights
            ...
            return {
                ...
                "global_average": (
                    statistics.mean(all_recent_scores) if all_recent_scores else 0.0
                ),
                "top_performing_agents": self._get_top_agents(5),
                "feedback_distribution": self._get_feedback_distribution(),
                "evaluation_batches_processed": len(
                    [b for b in self.evaluation_batches if b.processed]
                ),
            }
    ...
    def export_feedback_data(self, filepath: str, days_back: int = 30) -> None:
        ...
        logger.info(
            f"Exported {len(export_data['feedback_entries'])} feedback entries to {filepath}"
        )
    ...
    def _update_reward_function(self, feedback: FeedbackEntry) -> None:
        ...
        if agent_id not in self.reward_functions:
            ...
            self.reward_functions[agent_id] = self._create_simple_reward_function(
                agent_id
            )

    def _create_simple_reward_function(
        self, agent_id: str
    ...
    async def _process_agent_feedback(
        self, agent_id: str, feedback_list: List[FeedbackEntry]
    ) -> Dict[str, Any]:
    ...
    async def _update_reward_functions(
        self, agent_feedback: Dict[str, List[FeedbackEntry]]
    ) -> None:
    ...
    def _generate_improvement_suggestions(
        ...
        if len(negative_feedback) > len(feedback_list) * 0.3:
            suggestions.append(
                "High negative feedback rate - review action selection strategy"
            )
        ...
        if len(scores) > 2 and statistics.stdev(scores) > 0.5:
            suggestions.append(
                "Inconsistent performance - consider more stable decision making"
            )

        # Safety-related suggestions
        safety_violations = [
            f for f in feedback_list if f.feedback_type == FeedbackType.SAFETY_VIOLATION
        ]
