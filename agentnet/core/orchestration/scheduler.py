...
class TaskScheduler:
    ...
    def set_task_executor(
        self, executor: Callable[[str, str, str, Dict[str, Any]], Dict[str, Any]]
    ):
        """
    ...
    async def execute_graph(
        self, task_graph: TaskGraph, context: Optional[Dict[str, Any]] = None
    ) -> ExecutionResult:
        """
        ...
        if not task_graph.is_valid:
            raise ValueError(
                f"Cannot execute invalid task graph: {task_graph.validation_errors}"
            )
        ...
        self.logger.info(
            f"Starting DAG execution {execution_id} with {len(task_graph.nodes)} tasks"
        )
        ...
        try:
            ...
            while len(completed_tasks) < len(task_graph.nodes):
                ...
                ready_tasks = self._get_ready_tasks(
                    task_graph, completed_tasks, execution
                )
                ...
                if not ready_tasks:
                    ...
                    if not running_tasks:
                        ...
                        if failed_tasks:
                            ...
                            execution.error = (
                                f"Execution blocked by failed tasks: {failed_tasks}"
                            )
                            break
                        else:
                            ...
                            execution.status = ExecutionStatus.FAILED
                            execution.error = (
                                "No ready tasks found but execution not complete"
                            )
                            break
                ...
                if self.parallel_execution and len(ready_tasks) > 1:
                    await self._execute_tasks_parallel(
                        ready_tasks, task_graph, execution, context
                    )
                else:
                    for task_id in ready_tasks:
                        await self._execute_single_task(
                            task_id, task_graph, execution, context
                        )
            ...
            if execution.status == ExecutionStatus.RUNNING:
                if len(completed_tasks) == len(task_graph.nodes):
                    ...
                    self.logger.info(
                        f"DAG execution {execution_id} completed successfully"
                    )
                else:
                    execution.status = ExecutionStatus.FAILED
    ...
    def _get_ready_tasks(
        ...
        for node in task_graph.nodes:
            ...
            if (
                ...
                and task_result.attempt > self.max_retries
            ):
                continue
    ...
    async def _execute_single_task(
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            task_result.status = (
                TaskStatus.RUNNING if attempt == 1 else TaskStatus.RETRYING
            )
            task_result.start_time = time.time()

            self.logger.info(
                f"Executing task {task_id} with agent {agent_to_use} (attempt {attempt})"
            )
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    asyncio.create_task(
                        self._call_task_executor(
                            task_id, task_node.prompt, agent_to_use, task_context
                        )
                    ),
            ...
            if attempt < max_attempts:
                if self.enable_fallback and self.fallback_agent and attempt > 1:
                    ...
                    self.logger.info(
                        f"Switching to fallback agent {self.fallback_agent} for task {task_id}"
                    )
    ...
    async def _call_task_executor(
        self, task_id: str, prompt: str, agent: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    async def example_task_executor(
        task_id: str, prompt: str, agent: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def test_scheduler():
        ...
        for task_id, task_result in result.task_results.items():
            print(
                f"  {task_id}: {task_result.status} ({task_result.execution_time:.2f}s)"
            )
