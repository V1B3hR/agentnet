...
class DAGPlanner:
    ...
    def create_graph_from_dict(self, data: Dict[str, Any]) -> TaskGraph:
        ...
        try:
            ...
            task_graph = TaskGraph(nodes=nodes, metadata=data.get("metadata", {}))
            self._validate_graph(task_graph)
            return task_graph
            ...
            task_graph.validation_errors.append(
                f"Invalid task graph structure: {str(e)}"
            )
            return task_graph
    ...
    def _validate_graph(self, task_graph: TaskGraph) -> None:
        ...
        try:
            ...
            task_graph.validation_errors.append(
                f"Error building graph for cycle detection: {str(e)}"
            )
        ...
        if not root_nodes:
            task_graph.validation_errors.append(
                "Task graph must have at least one root node (node with no dependencies)"
            )
        ...
        if task_graph.is_valid:
            self.logger.info(
                f"Task graph validation successful: {len(task_graph.nodes)} nodes, {len(root_nodes)} root nodes"
            )
        else:
            self.logger.warning(
                f"Task graph validation failed: {task_graph.validation_errors}"
            )
    ...
    def get_execution_order(self, task_graph: TaskGraph) -> List[List[str]]:
        ...
        if not task_graph.is_valid:
            raise ValueError(
                f"Cannot get execution order for invalid task graph: {task_graph.validation_errors}"
            )
    ...
    def get_ready_nodes(
        self, task_graph: TaskGraph, completed_nodes: Set[str]
    ) -> List[str]:
        """
        ...
        if not task_graph.is_valid:
            raise ValueError(
                f"Cannot get ready nodes for invalid task graph: {task_graph.validation_errors}"
            )
    ...
    def analyze_graph(self, task_graph: TaskGraph) -> Dict[str, Any]:
        ...
        leaf_nodes = [
            node_id for node_id in nx_graph.nodes() if nx_graph.out_degree(node_id) == 0
        ]
