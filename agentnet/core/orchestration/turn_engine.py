...
class TurnEngine:
    ...
    async def execute_single_agent_session(
        ...
        try:
            ...
            while turn_number <= self.max_turns and round_number <= self.max_rounds:
    ...
    async def execute_multi_agent_session(
        ...
        self.active_sessions[session_id] = session
        logger.info(
            f"Starting multi-agent session {session_id} with {len(agents)} agents, mode={mode}"
        )
        ...
        try:
            ...
            while round_number <= self.max_rounds and turn_number <= self.max_turns:
            ...
            if self.policy_engine:
                ...
                if isinstance(violations, list) and violations:
                    ...
                    if hasattr(violations[0], "to_dict"):
                        turn_result.policy_violations = [
                            v.to_dict() for v in violations
                        ]
                    else:
                        turn_result.policy_violations = [
                            {"name": str(v), "severity": "unknown"} for v in violations
                        ]
                    logger.warning(
                        f"Policy violations in turn {turn_id}: {[str(v) for v in violations]}"
                    )
        ...
        for i, agent in enumerate(agents):
            ...
            if round_number == 1:
                if i == 0:
                    ...
                    prev_content = (
                        prev_turn.content if prev_turn else "the previous position"
                    )
                ...
                recent_arguments = "; ".join(
                    [t.content[:100] + "..." for t in recent_turns]
                )
    ...
    def _generate_round_robin_prompt(
        ...
        if round_number == 1:
            return (
                f"Discuss and analyze: {topic}. Provide your perspective and insights."
            )
        else:
    ...
    def _generate_followup_prompt(self, turn_result: TurnResult) -> str:
        ...
        content_snippet = (
            turn_result.content[:200] + "..."
            if len(turn_result.content) > 200
            else turn_result.content
        )
    ...
    def _extract_convergence_points(self, turns: List[TurnResult]) -> str:
        ...
        return (
            "; ".join(found_convergence)
            if found_convergence
            else "Working toward alignment"
        )
