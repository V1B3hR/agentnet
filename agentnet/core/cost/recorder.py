...
class CostRecorder:
    ...
    def record_inference_cost(
        ...
        logger.debug(f"Recorded cost: {cost_record.total_cost:.6f} for {agent_name}/{task_id}")
        return cost_record
    ...
    def get_records(
        ...
        while current_date <= end_date.date():
            filename = self.storage_dir / f"costs_{current_date.strftime('%Y-%m-%d')}.jsonl"
            if filename.exists():
...
class CostAggregator:
    ...
    def get_cost_trends(
        ...
        for record in records:
            ...
            daily_costs[date_key]["total_tokens"] += record.tokens_input + record.tokens_output
    ...
    def get_top_cost_agents(
        ...
        for stats in agent_stats.values():
            if stats["inference_count"] > 0:
                stats["avg_cost_per_inference"] = stats["total_cost"] / stats["inference_count"]
        
        # Sort by total cost and limit
        top_agents = sorted(agent_stats.values(), key=lambda x: x["total_cost"], reverse=True)
...
class TenantCostTracker:
    ...
    def __init__(self, recorder: CostRecorder):
        ...
        self.tenant_alerts: Dict[str, Dict[str, float]] = {}  # tenant_id -> {threshold -> alert_level}
