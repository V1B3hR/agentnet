...
class SecurityPolicy:
    ...
    def can_load_plugin(self, plugin_info: PluginInfo) -> bool:
        ...
        for permission in plugin_info.permissions:
            if permission in self.blocked_permissions:
                logger.warning(
                    f"Plugin {plugin_info.name} blocked by permission: {permission}"
                )
                return False
        ...
        if self.allowed_permissions:
            for permission in plugin_info.permissions:
                if permission not in self.allowed_permissions:
                    logger.warning(
                        f"Plugin {plugin_info.name} requires unavailable permission: {permission}"
                    )
                    return False
    ...
    def get_execution_restrictions(self, plugin_info: PluginInfo) -> Dict[str, Any]:
        ...
        if level == SecurityLevel.MINIMAL:
            ...
            restrictions["max_cpu_time_seconds"] = min(
                restrictions["max_cpu_time_seconds"], 10
            )
            ...
            restrictions["max_cpu_time_seconds"] = min(
                restrictions["max_cpu_time_seconds"], 30
            )
            restrictions["network_access"] = False
...
class PluginSandbox:
    ...
    def execute_in_sandbox(
        self, plugin_name: str, sandbox_env: Dict[str, Any], command: List[str]
    ...
    def validate_plugin_imports(
        self, plugin_code: str, restrictions: Dict[str, Any]
    ) -> List[str]:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        ...
                        if self._is_import_blocked(
                            module_name, allowed_imports, blocked_imports
                        ):
    ...
    def _is_import_blocked(
        self, module_name: str, allowed: Set[str], blocked: Set[str]
    ) -> bool:
        ...
        if allowed:
            if module_name not in allowed:
                ...
                allowed_prefixes = [
                    prefix for prefix in allowed if module_name.startswith(prefix + ".")
                ]
