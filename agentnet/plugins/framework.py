        ...
        def resolve(name: str):
            ...
            for dependency in self._dependencies.get(name, []):
                if dependency not in self._plugins:
                    raise ValueError(
                        f"Missing dependency: {dependency} for plugin {name}"
                    )
                resolve(dependency)
...
class PluginManager:
    ...
    def load_plugin(
        ...
        try:
            ...
            if self._security_policy and not self._security_policy.can_load_plugin(
                info
            ):
                raise PermissionError(
                    f"Security policy denies loading plugin: {plugin_name}"
                )
            ...
            spec = importlib.util.spec_from_file_location(
                f"plugin_{plugin_name}", entry_point
            )
    ...
    def initialize_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            for dep in dependencies[:-1]:  # Exclude self
                ...
                if not dep_plugin or dep_plugin.status not in [
                    PluginStatus.ACTIVE,
                    PluginStatus.INITIALIZED,
                ]:
                    logger.error(
                        f"Dependency {dep} not available for plugin {plugin_name}"
                    )
                    return False
        except Exception as e:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        for status in PluginStatus:
            status_counts[status.value] = sum(
                1 for p in plugin_status.values() if p["status"] == status.value
            )
