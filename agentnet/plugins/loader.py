...
from .security import SecurityPolicy, PluginSandbox
...
class PluginDiscovery:
    ...
    def __init__(self, search_paths: Optional[List[str]] = None):
        self.search_paths = [Path(p) for p in (search_paths or ["./plugins", "~/.agentnet/plugins"])]
        ...
        try:
            ...
            for field in required_fields:
                if field not in manifest_data:
                    logger.error(f"Plugin manifest missing required field '{field}': {manifest_file}")
                    return None
    ...
    def _validate_plugin_structure(self, plugin_dir: Path, plugin_info: PluginInfo) -> bool:
...
class PluginLoader:
    ...
    def load_plugin_from_path(self, plugin_path: Path, config: Optional[Dict[str, Any]] = None) -> Optional[Plugin]:
        ...
        try:
            ...
            if self.security_policy and not self.security_policy.can_load_plugin(plugin_info):
                logger.error(f"Security policy denies loading plugin: {plugin_info.name}")
                return None
    ...
    def _load_plugin_module(self, plugin_path: Path, plugin_info: PluginInfo, 
                           config: Optional[Dict[str, Any]]) -> Optional[Plugin]:
    ...
    def reload_plugin(self, plugin_path: Path, config: Optional[Dict[str, Any]] = None) -> Optional[Plugin]:
