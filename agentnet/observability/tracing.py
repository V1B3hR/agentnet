...
class TracingManager:
    ...
    def _setup_tracing(self, jaeger_endpoint: Optional[str], otlp_endpoint: Optional[str], console_export: bool):
    ...
    @contextmanager 
    def trace_agent_inference(self, agent_name: str, model: str, provider: str, session_id: Optional[str] = None):
        """
        ...
        with self.tracer.start_as_current_span("agent.inference", attributes=attributes) as span:
            try:
                yield span
                span.set_status(Status(StatusCode.OK))
            except Exception as e:
    ...
    @contextmanager
    def trace_monitor_sequence(self, monitor_name: str, rule_count: int, session_id: Optional[str] = None):
        """
        ...
        with self.tracer.start_as_current_span("monitor.sequence", attributes=attributes) as span:
            try:
                yield span
                span.set_status(Status(StatusCode.OK))
            except Exception as e:
    ...
    def trace_tool_invoke(self, tool_name: str, session_id: Optional[str] = None):
        ...
        with self.tracer.start_as_current_span("tool.invoke", attributes=attributes) as span:
            try:
                yield span
                span.set_status(Status(StatusCode.OK))
            except Exception as e:
    ...
    @contextmanager
    def trace_dag_node(self, node_id: str, node_type: str, agent_name: str, session_id: Optional[str] = None):
        """
        ...
        with self.tracer.start_as_current_span("dag.node.execute", attributes=attributes) as span:
            try:
                yield span
                span.set_status(Status(StatusCode.OK))
            except Exception as e:
...
def create_tracer(service_name: str = "agentnet", 
                 service_version: str = "0.4.0",
...
@contextmanager
def trace_agent_operation(agent_name: str, model: str, provider: str, session_id: Optional[str] = None):
...
@contextmanager
def trace_monitor_run(monitor_name: str, rule_count: int, session_id: Optional[str] = None):
