...
class DashboardDataCollector:
    ...
    def get_performance_metrics(
        ...
        filtered_data = [
            event
            for event in self._performance_data
            if start_time <= event["timestamp"] <= end_time
        ]
        ...
        requests_per_minute = (
            total_requests / duration_minutes if duration_minutes > 0 else 0.0
        )
    ...
    def get_violation_summary(
        ...
        filtered_data = [
            event
            for event in self._violation_data
            if start_time <= event["timestamp"] <= end_time
        ]
        ...
        recent_violations = sorted(
            filtered_data, key=lambda x: x["timestamp"], reverse=True
        )[:10]
    ...
    def get_session_metrics(
        ...
        filtered_data = [
            event
            for event in self._session_data
            if start_time <= event["timestamp"] <= end_time
        ]
        ...
        for session_events in sessions.values():
            ...
            max_round = max(
                (event.get("round_number", 0) or 0 for event in session_events),
                default=0,
            )
        ...
        convergence_rate = (
            (converged_sessions / total_sessions * 100) if total_sessions > 0 else 0.0
        )
    ...
    def generate_dashboard_html(
        self, time_range: TimeRange = TimeRange.LAST_24H
    ) -> str:
