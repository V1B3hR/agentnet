...
class LangChainCompatibilityLayer:
    ...
    def convert_message_to_langchain(self, message: Dict[str, Any]) -> BaseMessage:
        ...
        if message_type in ["user", "human"]:
            ...
            logger.warning(f"Unknown message type: {message_type}, defaulting to HumanMessage")
            return HumanMessage(content=content)
    ...
    def create_migration_guide(self, langchain_code: str) -> str:
        ...
        if "ChatOpenAI" in langchain_code or "OpenAI" in langchain_code:
            suggestions.append("2. Use AgentNet's OpenAI provider adapter instead of LangChain's ChatOpenAI")
        
        if "LLMChain" in langchain_code:
            suggestions.append("3. Replace LLMChain with AgentNet's reasoning capabilities")
...
class LangChainProviderAdapter(ProviderAdapter):
    ...
    def __init__(self, langchain_llm: Union[BaseLLM, BaseChatModel], compat_layer: LangChainCompatibilityLayer):
    ...
    async def ainfer(self, prompt: str, **kwargs) -> InferenceResult:
        ...
        try:
            ...
            if hasattr(self.langchain_llm, 'apredict'):
                ...
                logger.warning("LangChain LLM doesn't support async, falling back to sync")
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supports_async": hasattr(self.langchain_llm, 'apredict') or hasattr(self.langchain_llm, 'acall'),
