...
class ContextModel(BaseModel):
    ...
    short_term: List[str] = Field(
        default_factory=list, description="Short-term memory items"
    )
    semantic_refs: List[Dict[str, Any]] = Field(
        default_factory=list, description="Semantic memory references with scores"
    )
    episodic_refs: List[Dict[str, Any]] = Field(
        default_factory=list, description="Episodic memory references"
    )
...
class InputModel(BaseModel):
    ...
    context: ContextModel = Field(
        default_factory=ContextModel, description="Contextual information"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional input metadata"
    )
...
class OutputModel(BaseModel):
    ...
    style_insights: List[str] = Field(
        default_factory=list, description="Insights about applied reasoning style"
    )
...
class MonitorResultModel(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(
        None, description="Additional monitor details"
    )
    violations: List[Dict[str, Any]] = Field(
        default_factory=list, description="Any policy violations detected"
    )
...
class CostModel(BaseModel):
    ...
    tokens_per_dollar: Optional[float] = Field(
        None, ge=0, description="Token efficiency"
    )
...
class TimingModel(BaseModel):
    ...
    breakdown: Dict[str, float] = Field(
        default_factory=dict, description="Breakdown of latency by component"
    )
...
class TurnMessage(BaseModel):
    ...
    message_type: MessageType = Field(
        default=MessageType.TURN, description="Message type"
    )
    ...
    monitors: List[MonitorResultModel] = Field(
        default_factory=list, description="Monitor execution results"
    )
    ...
    def add_monitor_result(
        self, name: str, status: MonitorStatus, elapsed_ms: float, **kwargs
    ) -> None:
        """Add a monitor result to the message."""
        monitor_result = MonitorResultModel(
            name=name, status=status, elapsed_ms=elapsed_ms, **kwargs
        )
    ...
    def is_successful(self) -> bool:
        ...
        return not any(
            monitor.status == MonitorStatus.FAIL for monitor in self.monitors
        )
...
class MessageSchemaValidator:
    ...
    def get_schema_compliance_report(
        message: Union[TurnMessage, Dict[str, Any]],
    ) -> Dict[str, Any]:
        """Generate compliance report for a message."""
        report = {"valid": False, "errors": [], "warnings": [], "completeness": 0.0}
            ...
            for field in required_fields:
                if hasattr(msg, field) and getattr(msg, field) is not None:
                    ...
                    report["warnings"].append(
                        f"Missing or null required field: {field}"
                    )
...
def create_example_message() -> TurnMessage:
    ...
    return TurnMessage(
        ...
        monitors=[
            MonitorResultModel(
                name="keyword_guard", status=MonitorStatus.PASS, elapsed_ms=2.1
            )
        ],
        cost=CostModel(provider=CostProvider.OPENAI, model="gpt-4o", usd=0.01234),
        timing=TimingModel(
            started=1736981000.123, completed=1736981001.001, latency_ms=878
        ),
