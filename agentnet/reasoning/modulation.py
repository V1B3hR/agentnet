...
from .types import ReasoningType, ReasoningResult
...
class ReasoningStyleModulator:
    ...
    def modulate_style_for_reasoning(self, 
                                   base_style: Dict[str, float],
                                   reasoning_type: ReasoningType,
                                   modulation_strength: float = 0.5) -> Dict[str, float]:
        """
        ...
        for dimension, base_value in base_style.items():
            if dimension in optimal_profile:
                ...
                modulated_value = base_value * (1 - modulation_strength) + optimal_value * modulation_strength
                modulated_style[dimension] = min(1.0, max(0.0, modulated_value))
            else:
                modulated_style[dimension] = base_value
        
        logger.debug(f"Style modulated for {reasoning_type}: {base_style} → {modulated_style}")
...
class ReasoningAwareStyleInfluence:
    ...
    def apply_reasoning_aware_style_influence(self,
                                            base_result: Dict[str, Any],
                                            task: str,
                                            base_style: Dict[str, float],
                                            reasoning_result: Optional[ReasoningResult] = None,
                                            modulation_strength: float = 0.3) -> Dict[str, Any]:
        """
        ...
        if reasoning_result:
            modulated_style = self.modulator.modulate_style_for_reasoning(
                base_style,
                reasoning_result.reasoning_type,
                modulation_strength
            )
        ...
        styled_result = self._apply_general_style_influence(styled_result, modulated_style, task)
    ...
    def _apply_reasoning_specific_enhancements(self,
                                             result: Dict[str, Any],
                                             reasoning_result: ReasoningResult,
                                             modulated_style: Dict[str, float]) -> Dict[str, Any]:
        ...
        enhanced_confidence = (confidence + reasoning_confidence + style_alignment_bonus) / 3.0
        ...
        if reasoning_result.reasoning_steps:
            insights.extend([f"Reasoning: {step}" for step in reasoning_result.reasoning_steps[:2]])
    ...
    def _apply_general_style_influence(self,
                                     result: Dict[str, Any],
    ...
    def _calculate_style_alignment_bonus(self,
                                       reasoning_type: ReasoningType,
                                       style: Dict[str, float]) -> float:
    ...
    def create_reasoning_context(self,
                               task: str,
        ...
        context = {
            ...
            "suggested_reasoning_types": self.modulator.suggest_reasoning_types(agent_style),
    ...
    def _extract_style_preferences(self, style: Dict[str, float]) -> Dict[str, Any]:
        ...
        preferences = {
            ...
            "reasoning_depth": "deep" if max(logic_weight, analytical_weight) > 0.8 else "moderate",
