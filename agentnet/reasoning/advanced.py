...
class StepValidation:
    ...
    def validate_step(
        self, step: ReasoningStep, context: Optional[Dict[str, Any]] = None
    ) -> ValidationResult:
...
class ChainOfThoughtReasoning(BaseReasoning):
    ...
    def _form_conclusion(self, steps: List[ReasoningStep], task: str) -> str:
        ...
        valid_steps = [
            s for s in steps if s.validation_result == ValidationResult.VALID
        ]
...
class MultiHopReasoning(BaseReasoning):
    ...
    def reason(
            ...
            for i in range(len(entities)):
                for j in range(i + 1, len(entities)):
                    path = self.knowledge_graph.find_path(
                        entities[i], entities[j], self.max_hops
                    )
                    if path:
                        paths.append(path)
        ...
        if conclusions:
            ...
            final_content = (
                f"Multi-hop reasoning could not establish clear connections for: {task}"
            )
            confidence = 0.4
    ...
    def _build_default_graph(self, entities: List[str], task: str) -> None:
        ...
        for i in range(len(entities)):
            for j in range(i + 1, len(entities)):
                ...
                if (
                    abs(task_lower.find(entities[i]) - task_lower.find(entities[j]))
                    < 50
                ):
                    edge = KnowledgeEdge(
...
class CounterfactualReasoning(BaseReasoning):
    ...
    def reason(
        ...
        if analyses:
            ...
            conclusion = (
                f"Unable to generate meaningful counterfactual scenarios for: {task}"
            )
        ...
        if "because" in causal_claim.lower():
            ...
            if len(parts) >= 2:
                ...
                counterfactuals.append(
                    f"If {cause} had not occurred, then {effect} would not have happened"
                )
                counterfactuals.append(
                    f"If {cause} were stronger, then {effect} would be more pronounced"
                )
...
class AdvancedReasoningEngine:
    ...
    def advanced_reason(
        self, task: str, reasoning_mode: str, context: Optional[Dict[str, Any]] = None
    ) -> ReasoningResult:
    ...
    def auto_select_advanced_mode(self, task: str) -> str:
        ...
        if any(
            word in task_lower
            for word in ["connect", "relationship", "between", "through"]
        ):
            return "multi_hop"
        ...
        if any(
            word in task_lower
            for word in ["what if", "would have", "counterfactual", "alternative"]
        ):
            return "counterfactual"

        # Symbolic indicators
        if any(
            word in task_lower for word in ["logical", "proof", "theorem", "formal"]
        ):
            return "symbolic"
