...
class StreamingCollaborator:
    ...
    def create_session(
        ...
        **kwargs,
        ...
        session = CollaborationSession(
            mode=mode, participants=participants or [], **kwargs
        )
        ...
        logger.info(
            f"Created collaboration session {session.session_id} with mode {mode.value}"
        )
        return session
    ...
    async def _notify_partial_update(
    ...
    async def _notify_response_complete(
        self, session_id: str, response: PartialResponse
    ) -> None:
    ...
    def _handle_partial_update(
        self, stream_id: str, partial_data: Dict[str, Any], result: ParseResult
    ) -> None:
    ...
    def _handle_complete_response(
        self, stream_id: str, parsed_data: Dict[str, Any], result: ParseResult
    ) -> None:
    ...
    def _handle_parse_error(
        self, stream_id: str, error: str, result: ParseResult
    ) -> None:
    ...
    def get_session_stats(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "avg_response_length": sum(len(r.content) for r in session.responses)
            / max(len(session.responses), 1),
        }
