    ...
    async def handle(self, data: Any, context: Dict[str, Any]) -> Any:
        ...
        if stream_id not in self._active_streams:
            self._active_streams[stream_id] = StreamContext(
                session_id=session_id, agent_id=agent_id, stream_id=stream_id
            )
        ...
        async with self._coordination_lock:
            ...
            if len(self._active_streams) > self.max_concurrent_streams:
                logger.warning(
                    f"Too many concurrent streams ({len(self._active_streams)}), throttling"
                )
                await asyncio.sleep(0.1)

            # Process the data
            processed_data = await self._process_collaborative_data(
                data, stream_context, context
            )

        return processed_data

    async def _process_collaborative_data(
        self, data: Any, stream_context: StreamContext, context: Dict[str, Any]
    ) -> Any:
...
class ErrorHandler(StreamHandler):
    ...
    async def _handle_error(
        self, error: Any, stream_id: str, context: Dict[str, Any]
    ) -> Any:
        ...
        logger.warning(
            f"Stream error in {stream_id} (attempt {error_count}): {error_msg}"
        )
        ...
        error_response = {
            "error": True,
            "error_type": (
                type(error).__name__ if isinstance(error, Exception) else "StreamError"
            ),
        ...
        logger.error(
            f"Stream {stream_id} failed after {error_count} attempts: {error_msg}"
        )
        return error_response

    async def _process_safe_data(
        self, data: Any, stream_id: str, context: Dict[str, Any]
    ) -> Any:
    ...
    async def _recover_partial_json(
    ...
    async def _recover_connection(
        ...
        return {"recovered": True, "recovery_type": "connection_reset"}

    def register_recovery_strategy(
        self, error_type: str, recovery_func: Callable
    ) -> None:
...
class FilterHandler(StreamHandler):
    ...
    def __init__(
        self, name: str = "filter_handler", filters: Optional[List[Callable]] = None
    ):
    ...
    async def handle(self, data: Any, context: Dict[str, Any]) -> Any:
        ...
        for i, transform_func in enumerate(self.transformers):
            try:
                ...
                transformer_name = getattr(
                    transform_func, "__name__", f"transformer_{i}"
                )
