...
class AuditStorage:
    ...
    def _init_database(self) -> None:
        ...
        with sqlite3.connect(self.storage_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_agent_id ON audit_events(agent_id)")
    ...
    def get_events(self, query: Optional[AuditQuery] = None, limit: int = 100) -> List[AuditEvent]:
    ...
    def get_event_statistics(self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    def export_events(self, filepath: str, query: Optional[AuditQuery] = None, format: str = "json") -> None:
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.storage_path) as conn:
            ...
            cursor = conn.execute("SELECT MIN(timestamp), MAX(timestamp) FROM audit_events")
            time_range = cursor.fetchone()
            
            # Get database file size
            db_size = self.storage_path.stat().st_size if self.storage_path.exists() else 0
