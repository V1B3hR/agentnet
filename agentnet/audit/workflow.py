...
class AuditLogger:
    ...
    def _get_automatic_compliance_tags(self, event: AuditEvent) -> List[str]:
        ...
        if event.event_type in [AuditEventType.USER_LOGIN, AuditEventType.USER_LOGOUT, AuditEventType.PERMISSION_GRANT]:
            tags.append("SOC2_SECURITY")
        
        if event.event_type in [AuditEventType.DATA_ACCESS, AuditEventType.EXPORT_EVALUATION]:
    ...
    def log_agent_inference(self, agent_id: str, session_id: str, user_id: Optional[str] = None, **kwargs) -> None:
    ...
    def log_policy_violation(self, violation_details: Dict[str, Any], session_id: str, agent_id: str, **kwargs) -> None:
        """Log policy violation event."""
        severity = AuditSeverity.HIGH if violation_details.get("severity") == "severe" else AuditSeverity.MEDIUM
    ...
    def log_export_evaluation(self, classification_level: str, export_allowed: bool, destination: str, **kwargs) -> None:
    ...
    def log_user_action(self, user_id: str, action: str, details: Dict[str, Any] = None, **kwargs) -> None:
        """Log user action event."""
        event_type = AuditEventType.USER_LOGIN if "login" in action.lower() else AuditEventType.DATA_ACCESS
    ...
    def log_security_alert(self, alert_type: str, details: Dict[str, Any], severity: AuditSeverity = AuditSeverity.HIGH) -> None:
    ...
    def register_event_handler(self, event_type: AuditEventType, handler: callable) -> None:
...
class AuditWorkflow:
    ...
    def trigger_workflow(self, workflow_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def add_compliance_rule(self, rule_name: str, condition: callable, action: callable) -> None:
