...
from ..observability.metrics import get_global_metrics, MetricsCollector
...
class InstrumentedProviderMixin:
    ...
    def _instrument_inference(self, method_name: str, sync: bool = True):
        ...
        def decorator(func):
            if sync:
                @wraps(func)
                def wrapper(prompt: str, agent_name: str = "Agent", 
                ...
                @wraps(func)
                async def async_wrapper(prompt: str, agent_name: str = "Agent",
                                      session_id: Optional[str] = None, **kwargs):
    ...
    def _execute_instrumented_inference(self, func, method_name: str, prompt: str, 
                                      agent_name: str, session_id: Optional[str], **kwargs):
        ...
        try:
            ...
            with trace_agent_operation(agent_name, self.model_name, self.provider_name, session_id) as span:
                # Add span attributes
                ...
                logger.log_agent_inference(
                    self.model_name, self.provider_name, total_tokens, duration_ms,
    ...
    async def _execute_instrumented_inference_async(self, func, method_name: str, prompt: str,
                                                   agent_name: str, session_id: Optional[str], **kwargs):
...
class InstrumentedProviderAdapter(InstrumentedProviderMixin, ProviderAdapter):
    ...
    def infer(self, prompt: str, agent_name: str = "Agent", 
    ...
    async def async_infer(self, prompt: str, agent_name: str = "Agent",
                         session_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
    ...
    class InstrumentedProvider(InstrumentedProviderMixin, provider_class):
        def __init__(self, *args, **kwargs):
            ...
            if hasattr(self, 'infer'):
                ...
                self.infer = self._instrument_inference('infer', sync=True)(original_infer)
            
            # Instrument the async_infer method if it exists
            if hasattr(self, 'async_infer'):
                original_async_infer = self.async_infer
                self.async_infer = self._instrument_inference('async_infer', sync=False)(original_async_infer)
