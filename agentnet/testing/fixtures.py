...
class AgentFixtures:
    ...
    def create_agent_group(
        ...
        if group_type == "debate_pair":
            profiles = [
                self.get_profile("Advocate"),
                self.get_profile("Critic")
            ]
            ...
            # Random selection
            profiles = random.sample(self._profiles, min(size or 3, len(self._profiles)))
...
class ScenarioGenerator:
        ...
        **kwargs
        ...
        if category in self._prompt_templates:
            ...
            template_vars = {
                ...
                'problem_description': kwargs.get('problem_description', f"Issues with {topic}"),
                ...
                'scale_requirement': kwargs.get('scale_requirement', 'high availability'),
                **kwargs
            }
            try:
    ...
    def get_scenarios_for_duration(self, max_duration_seconds: float) -> List[TestScenario]:
