...
class IntegrationTestSuite:
        ...
        test_cases.append(
            MultiAgentTestCase(
                ...
                agent_configs=[
                    {
                        "name": "ResearchAgent",
                        "style": {"logic": 0.8, "analytical": 0.9},
                    }
                ],
    ...
    async def run_all_tests(
        self, include_observability: bool = True, parallel_execution: bool = False
    ) -> Dict[str, Any]:
    ...
    async def _run_tests_sequential(
        self, include_observability: bool
    ) -> List[Dict[str, Any]]:
        ...
        for i, test_case in enumerate(self._test_cases, 1):
            logger.info(
                f"Running integration test {i}/{len(self._test_cases)}: {test_case.name}"
            )
            ...
            logger.info(
                f"{status} {test_case.name} ({result['duration_seconds']:.2f}s)"
            )
    ...
    async def _execute_test_case(
        self, test_case: MultiAgentTestCase, include_observability: bool
    ) -> Dict[str, Any]:
    ...
    async def _create_test_agents(self, test_case: MultiAgentTestCase) -> List:
        ...
        for config in test_case.agent_configs:
            ...
            agent = AgentNet(name=config["name"], style=config["style"], **agent_config)
    ...
    async def _execute_dialogue_scenario(
        self, test_case, agents, latency_tracker, token_tracker
    ):
        ...
        if hasattr(agents[0], "dialogue_with"):
            result = agents[0].dialogue_with(
                agents[1], test_case.test_prompt, rounds=test_case.rounds
            )
        else:
    ...
    async def _execute_debate_scenario(
        self, test_case, agents, latency_tracker, token_tracker
    ):
        ...
        debate_prompt = (
            f"Debate this topic: {test_case.test_prompt}. Take opposing positions."
        )
        ...
        if hasattr(agents[0], "dialogue_with"):
            result = agents[0].dialogue_with(
                agents[1], debate_prompt, rounds=test_case.rounds
            )
        else:
        ...
        return {
            ...
            "result": result,
            "agent_results": [
                {"agent": agent.name, "participated": True} for agent in agents[:2]
            ],
        }
    ...
    async def _execute_collaboration_scenario(
        ...
        if hasattr(agents[0], "multi_party_dialogue"):
            ...
            for i, agent in enumerate(agents):
                ...
                contributions.append(
                    {
                        "agent": agent.name,
                        "contribution": response.get("result", {}).get(
                            "content", "No contribution"
                        ),
                    }
                )
        ...
        return {
            ...
            "result": result,
            "agent_results": [
                {"agent": agent.name, "participated": True} for agent in agents
            ],
        }

    async def _execute_tool_usage_scenario(
        self, test_case, agents, latency_tracker, token_tracker
    ):
    ...
    async def _execute_streaming_scenario(
        self, test_case, agents, latency_tracker, token_tracker
    ):
    ...
    def _collect_observability_data(
        self, latency_tracker, token_tracker
    ) -> Dict[str, Any]:
        ...
        if latency_tracker:
            ...
            if measurements:
                data["latency"] = {
                    ...
                    "avg_latency_ms": sum(m.total_latency_ms for m in measurements)
                    / len(measurements),
    ...
    def _evaluate_success_conditions(
        ...
        agent_results = scenario_result.get("agent_results", [])
        participated_count = len(
            [r for r in agent_results if r.get("participated", False)]
        )
        ...
        for condition in test_case.success_conditions:
            ...
            passed = self._evaluate_condition(
                condition, scenario_result, observability_data
            )
    ...
    def _generate_test_summary(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        avg_duration = (
            sum(r["duration_seconds"] for r in results) / total_tests if results else 0
        )
