...
class PerformanceReporter:
    ...
    def _generate_executive_summary(
        ...
        if benchmark_results:
            ...
            successful_benchmarks = [
                r for r in benchmark_results if r.passed_thresholds
            ]
            summary["benchmark_success_rate"] = len(successful_benchmarks) / len(
                benchmark_results
            )
            summary["avg_turn_latency_ms"] = sum(
                r.avg_turn_latency_ms for r in benchmark_results
            ) / len(benchmark_results)

            # Key findings from benchmarks
            if summary["benchmark_success_rate"] < 0.8:
                summary["key_findings"].append(
                    "⚠️ Multiple benchmarks failed to meet performance thresholds"
                )

            high_latency_benchmarks = [
                r for r in benchmark_results if r.avg_turn_latency_ms > 2000
            ]
            if high_latency_benchmarks:
                summary["key_findings"].append(
                    f"🐌 {len(high_latency_benchmarks)} benchmarks showed high latency (>2s)"
                )
        ...
        if token_tracker:
            ...
            if system_overview:
                summary["total_tokens_analyzed"] = system_overview["overview"][
                    "total_tokens"
                ]
                summary["overall_efficiency_score"] = system_overview["overview"][
                    "avg_efficiency"
                ]

                if summary["overall_efficiency_score"] < 0.6:
                    summary["key_findings"].append(
                        "📉 Token efficiency is below optimal levels"
                    )
        ...
        if latency_tracker:
            ...
            if measurements:
                avg_latency = sum(m.total_latency_ms for m in measurements) / len(
                    measurements
                )
                if avg_latency > 1500:
                    summary["key_findings"].append(
                        "⏱️ Average turn latency exceeds recommended thresholds"
                    )
    ...
    def _analyze_benchmarks(
        ...
        analysis = {
            "summary": {
                "total_benchmarks": len(benchmark_results),
                "passed_benchmarks": len(
                    [r for r in benchmark_results if r.passed_thresholds]
                ),
                "failed_benchmarks": len(
                    [r for r in benchmark_results if not r.passed_thresholds]
                ),
            },
            "latency_metrics": {},
            "throughput_metrics": {},
    ...
    def _analyze_latency(self, latency_tracker: LatencyTracker) -> Dict[str, Any]:
        ...
        for agent_id in agent_ids:
            ...
            agent_issues = latency_tracker.identify_performance_issues(
                agent_id=agent_id
            )
    ...
    def _analyze_tokens(self, token_tracker: TokenUtilizationTracker) -> Dict[str, Any]:
        ...
        for agent_id in system_overview["agents"]:
            ...
            agent_opportunities = token_tracker.identify_optimization_opportunities(
                agent_id
            )
    ...
    def _analyze_trends(
        ...
        trends = {"benchmark_trends": {}, "latency_trends": {}, "token_trends": {}}
        ...
        if len(benchmark_results) > 1:
            ...
            trends["benchmark_trends"] = {
                "latency_trend": (
                    "improving" if latency_trend[-1] < latency_trend[0] else "degrading"
                ),
                "success_trend": (
                    "improving" if success_trend[-1] > success_trend[0] else "degrading"
                ),
                "latest_vs_first": {
                    "latency_change_pct": (
                        (latency_trend[-1] - latency_trend[0]) / latency_trend[0]
                    )
                    * 100,
                    "success_change_pct": (
                        (success_trend[-1] - success_trend[0]) / success_trend[0]
                    )
                    * 100,
                },
            }
    ...
    def _generate_recommendations(
        ...
        if benchmark_analysis:
            avg_latency = benchmark_analysis.get("latency_metrics", {}).get(
                "avg_latency_ms", 0
            )
            avg_success_rate = benchmark_analysis.get("success_metrics", {}).get(
        ...
        if token_tracker:
            token_recommendations = (
                token_tracker.generate_optimization_recommendations()
            )
    ...
    def _format_markdown_report(self, report_data: Dict[str, Any]) -> str:
        ...
        md.append(
            f"- **Benchmark Success Rate:** {summary['benchmark_success_rate']:.1%}"
        )
        ...
        md.append(
            f"- **Overall Efficiency Score:** {summary['overall_efficiency_score']:.3f}"
        )
        md.append("")
