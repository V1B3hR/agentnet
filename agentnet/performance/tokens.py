...
class TokenUtilizationTracker:
    ...
    def record_token_usage(
        ...
        tokens_per_second = (
            total_tokens / processing_time if processing_time > 0 else 0.0
        )
        cost_per_token = self._cost_models.get(model_name, self._cost_models["default"])

        # Estimate token efficiency (simplified heuristic)
        effective_tokens = self._estimate_effective_tokens(
            output_tokens, output_quality_score
        )
        redundant_tokens = total_tokens - effective_tokens
        token_efficiency_ratio = (
            effective_tokens / total_tokens if total_tokens > 0 else 0.0
        )
    ...
    def _estimate_effective_tokens(
        self, output_tokens: int, quality_score: float
    ) -> int:
    ...
    def _update_category_tracking(
        self, agent_id: str, category_breakdown: Dict[TokenCategory, int]
    ) -> None:
    ...
    def get_agent_token_summary(self, agent_id: str) -> Dict[str, Any]:
        ...
        # Cost analysis
        total_cost = sum(
            m.total_tokens * m.cost_per_token / 1000 for m in agent_metrics
        )
    ...
    def get_system_token_overview(self) -> Dict[str, Any]:
        ...
        # Cost totals
        total_cost = sum(
            m.total_tokens * m.cost_per_token / 1000 for m in self._metrics
        )

        # Agent breakdown
        agent_ids = list(set(m.agent_id for m in self._metrics))
        agent_summaries = {
            agent_id: self.get_agent_token_summary(agent_id) for agent_id in agent_ids
        }

        # Time-based analysis (last 24h, 1h, etc.)
        now = time.time()
        recent_metrics = [
            m for m in self._metrics if (now - m.timestamp) < 3600
        ]  # Last hour
    ...
    def identify_optimization_opportunities(
        ...
        for metric in metrics:
            # High redundancy
            redundancy_ratio = (
                metric.redundant_tokens / metric.total_tokens
    ...
    def export_metrics(
        self, format: str = "json", agent_id: Optional[str] = None
    ) -> Any:
            ...
            return [
                {
                    ...
                    "category_breakdown": {
                        k.value: v for k, v in m.category_breakdown.items()
                    },
                }
                for m in metrics
            ]
    ...
    def set_cost_model(self, model_name: str, cost_per_1k_tokens: float) -> None:
        ...
        logger.info(
            f"Set cost model for {model_name}: ${cost_per_1k_tokens:.4f}/1K tokens"
        )
